...
{
    /// <summary>
    /// A key this game knows how to bind.
    ///
    /// <para>A closed set on purpose. It is everything the game reads today and nothing else:
    /// the rebind panel can offer the whole set without a fallback for "a key the list does
    /// not know", and <see cref="HotkeyDirector.UnknownKeyCount"/> keeps the capture loop and
    /// the set honest about each other. Escape is <b>not</b> a member — Escape is the unwind
    /// rule (<see cref="SettingsDirector.Escape"/>) and the way out of a rebind, so it is not
    /// a thing anything else may claim. Modifiers are not members either: Shift is the fast
    /// modifier and the additive-selection modifier, and a rebinding scheme that can put an
    /// action <i>on</i> a modifier is a different, larger scheme.</para>
    /// </summary>
    public enum HudKey
    {
        None = 0,

        // Letters the game reads. Not the whole alphabet: a key here is a key some action
        // defaults to, and the letters the command bar avoided (W, A, S, D, Q, E...) are all
        // here because they were already taken by the camera.
        A, B, C, D, E, F, M, Q, R, S, V, W, X,
    ...
    public sealed class HotkeyDirector
    {
        ...
        public HotkeyAction? OwnerOf(HudKey key)
        {
            ...
            foreach ((HotkeyAction action, _) in Defaults)
        ...
        /// <summary>
        /// Bindings two stored preferences landed on the same key, from the last
        /// <see cref="UseStore"/>. Reported rather than silently resolved — except that a
        /// file that cannot be read back is worse than a deterministic rule, so the earlier
        /// action in draw order keeps the key and the later one is left unbound, and this
        /// list is who lost.
        /// </summary>
        ...
        /// itself. The empty slot's name is the panel's business, not the key's.
        /// </summary>
        ...
        ///
        /// <para>A stored line nobody can parse is dropped rather than refused: a key
        /// renamed between builds must not cost a player their whole binding file. A line
        /// that collides with another is settled by draw order and reported in
        /// <see cref="LoadConflicts"/>.</para>
        /// </summary>
        public void UseStore(ISettingsStore store)
        {
            ...
            foreach ((HotkeyAction action, _, _) in Defaults)
            {
                ...
                _bindings[action][0] = HudKey.None;
                _bindings[action][1] = HudKey.None;
                int slot = 0;
            ...
            // Defaults and stored lines are settled in one walk, in draw order: a stored
            // key may collide with another action's *default* just as easily as with its
            // stored one, and the rule has to cover both to be a rule at all.
            _loadConflicts.Clear();
            var taken = new HashSet<HudKey>();
            foreach ((HotkeyAction action, _, _) in Defaults)
                for (int slot = 0; slot < SlotCount; slot++)
                {
                    HudKey key = _bindings[action][slot];
                    if (key == HudKey.None || taken.Add(key)) continue;
                    _bindings[action][slot] = HudKey.None;
