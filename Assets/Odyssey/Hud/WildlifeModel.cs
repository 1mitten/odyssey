...
{
    /// <summary>One animal on the board, as the Wildlife panel lists it (design 30 §6).</summary>
    public struct WildlifeRow
    {
        public PawnId Id;
        public int Kind;
        ...
        /// <summary>Cells from the colony, Chebyshev, where the colony is where its people stand.</summary>
        public int Away;

        public bool Selected;
    }

    /// <summary>How many of a kind are on the board: the panel's header.</summary>
    public struct WildlifeCount
    {
        public int Kind;
        public string KindKey;
        public int Count;
    ...
    ///
    /// <para><b>The colony is where its people stand.</b> The panel says how far away an animal
    /// is, and "from what" has to be answered by the frame alone: the mean of the colonists'
    /// cells is a point the frame carries, the start cell is not, and a colony that has moved
    /// house has moved its animals' distances with it. With no colonists the distance is from
    /// the board's origin, which is a number rather than a lie.</para>
    ///
    ...
    /// </summary>
    public sealed class WildlifeModel
    {
        /// <summary>Every animal, sorted: kind, then distance, then id.</summary>
        public readonly List<WildlifeRow> All = new List<WildlifeRow>();

        /// <summary>The current page of <see cref="All"/>.</summary>
        public readonly List<WildlifeRow> Rows = new List<WildlifeRow>();

        /// <summary>One entry per kind on the board, ascending by kind.</summary>
        public readonly List<WildlifeCount> Counts = new List<WildlifeCount>();

        public int Page { get; private set; }

        public int PageCapacity { get; set; } = WildlifeLayout.RowsPerPage;
        ...
        readonly List<WildlifeRow> _scratch = new List<WildlifeRow>();
        ...
        public void Refresh(WorldSnapshot snapshot, IReadOnlyList<PawnId> selected)
        {
            ...
            // The colony's centre first, from the people in the frame.
            long sumX = 0, sumZ = 0;
            int people = 0;
            ...
            _scratch.Sort(ByKindThenDistance);
            ...
            for (int i = 0; i < All.Count; i++)
            {
                if (Counts.Count > 0 && Counts[Counts.Count - 1].Kind == All[i].Kind)
                {
                    WildlifeCount last = Counts[Counts.Count - 1];
                    last.Count++;
                    Counts[Counts.Count - 1] = last;
                }
                else
                {
                    Counts.Add(new WildlifeCount { Kind = All[i].Kind, KindKey = All[i].KindKey, Count = 1 });
                }
            }
        ...
        static readonly Comparison<WildlifeRow> ByKindThenDistance = (a, b) =>
        {
