...
namespace Odyssey.Tests.Sim
{
    ...
    public class AttackDriverTests
    {
        ...
        /// step while the world that was saved walked on. Three marauders on two drafted colonists: the
        ...
        public void ASaveTakenMidStepInReachResumesTheSame()
        {
            ...
            Assert.That(Draft(colony, a), Is.EqualTo(IntentRejection.None));
            Assert.That(Draft(colony, b), Is.EqualTo(IntentRejection.None));
            for (int i = 0; i < 3; i++) Spawn(colony, PawnKindIndex.Marauder, Near(colony, 9, -2 + 2 * i));
