...
{
    /// <summary>
    /// Marauders at a building with fewer free sides than marauders (design 33 §19; owner,
    ...
    /// a terrace edge most of those are air over the step below, so that wall had one. One marauder
    ...
    /// and a marauder whose every side is taken thinks again at once.</para>
    /// </summary>
    public class MarauderSideTests
    {
        ...
        /// <summary>
        /// The owner's case, built small: a colonist sealed in eight walls, so no marauder can reach
        ...
        /// the cells round it being air over the ground below; and three marauders on the ground
        /// beyond it, to whom that wall is the nearest colony building by far.
        /// </summary>
        static (ColonyWorld colony, Pawn[] marauders, int edge, int side) AWallOnTheEdgeOfAStep()
        {
            ...
            var marauders = new[]
            {
                Spawn(colony, PawnKindIndex.Marauder, Size.Index(s.X + 13, s.Z - 1, s.Y)),
                Spawn(colony, PawnKindIndex.Marauder, Size.Index(s.X + 13, s.Z, s.Y)),
                Spawn(colony, PawnKindIndex.Marauder, Size.Index(s.X + 13, s.Z + 1, s.Y)),
            };
            return (colony, marauders, edge, side);
        }
        ...
        /// the run; and no marauder spends more than two ticks attacking a building it has no side of.
        /// Before the fix two of them waited the whole run, from a side the third was holding.
        /// </summary>
        [Test]
        public void ThreeMaraudersAtAWallWithOneSideDoNotStandAbout()
        {
            var (colony, marauders, edge, side) = AWallOnTheEdgeOfAStep();
            var ctx = colony.Pawns;
            int handle = colony.Grid.Edifice[edge];

            // The controls: the wall on the edge has exactly one side, a marauder can get to it, and
            ...
            for (int dz = -1; dz <= 1; dz++)
            for (int dx = -1; dx <= 1; dx++)
                if ((dx != 0 || dz != 0) && ctx.Nav.Grid.CanEnter(Size.Index(e.X + dx, e.Z + dz, e.Y), TraverseMode.Marauder)) sides++;
            Assert.That(sides, Is.EqualTo(1), "the control: the wall on the step's edge has one side on its layer");
            Assert.That(BuildingTargets.CanReach(ctx, marauders[0], wall, TraverseMode.Marauder), Is.True,
                "the control: a marauder can get up the step to that side");
            Pawn colonist = ctx.Pawns.All[0];
            Assert.That(ctx.Reachable(marauders[0], colonist.Cell, TraverseMode.Marauder), Is.False,
                "the control: the colonist is out of every marauder's reach");

            var tape = new Tape();
            var waiting = new int[marauders.Length];
            var longest = new int[marauders.Length];
            bool edgeTaken = false;
            for (int t = 0; t < 2_400; t++)
            {
                tape.Tick(colony, 1);
                for (int i = 0; i < marauders.Length; i++)
                {
                    Pawn m = marauders[i];
            ...
            for (int i = 0; i < marauders.Length; i++)
            {
                Assert.That(tape.Swings(marauders[i]), Is.Not.Empty, $"marauder {i} never swung at anything: it stood about");
                // Two ticks at most: a job given at the end of a tick has no destination until its
                // driver's first look, and two marauders choosing one side in the same tick costs
                // the slower one a second. Before the fix this was the whole run.
                Assert.That(longest[i], Is.LessThanOrEqualTo(2),
                    $"marauder {i} waited {longest[i]} ticks at a building whose every side was taken");
                foreach (var swing in tape.Swings(marauders[i])) struck.Add(Size.Index(swing.Cell));
            }
            Assert.That(struck.Count, Is.GreaterThanOrEqualTo(3), "three marauders, and fewer than three walls struck");
            TestContext.WriteLine($"longest waits {string.Join(", ", longest)}; {struck.Count} cells struck");
            _ = side;
        }

        /// <summary>
        /// The one rule, asked directly: with the wall's one side held by another marauder the wall
        ...
        public void AWallWhoseOnlySideIsHeldIsPassedOverForOneWithASide()
        {
            ...
            Assert.That(BuildingTargets.TryNearestColonyTarget(ctx, marauders[1], TraverseMode.Marauder, out BuildingTarget first), Is.True);
            Assert.That(first.Handle, Is.EqualTo(handle), "the control: with its side free, the wall on the edge is the nearest");
            Assert.That(BuildingTargets.HasAFreeSide(ctx, marauders[1], wall, TraverseMode.Marauder), Is.True);

            // The first marauder takes the side, as the driver would: attacking the wall, standing on it.
            Pawn holder = marauders[0];
            Stand(colony, holder, side);
            var job = holder.JobBuffer;
            Assert.That(AttackJob.FillBuilding(ctx, holder, wall, job, TraverseMode.Marauder), Is.True);
            Assert.That(colony.Jobs.StartJob(holder, job, colony.World.CurrentTick), Is.True);
            Assert.That(Melee.Holds(ctx, marauders[1], side), Is.True, "the control: the side is held");

            Assert.That(BuildingTargets.CanReach(ctx, marauders[1], wall, TraverseMode.Marauder), Is.True,
                "reach is the order's question and is unchanged: a side can still be got to");
            Assert.That(BuildingTargets.HasAFreeSide(ctx, marauders[1], wall, TraverseMode.Marauder), Is.False,
                "a side another marauder holds was counted as free");
            Assert.That(BuildingTargets.TryNearestColonyTarget(ctx, marauders[1], TraverseMode.Marauder, out BuildingTarget next), Is.True,
