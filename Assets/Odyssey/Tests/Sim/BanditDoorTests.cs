...
{
    /// <summary>
    /// Doors hold bandits out, and bandits leave beds alone (design 33 §16; the owner,
    /// 2026-09-24: "do what you recommend"). A bandit moves in <see cref="TraverseMode.Bandit"/> —
    ...
    public class BanditDoorTests
    {
        ...
        /// <summary>Knock the colonist down with one exact blow from the bandit, so nobody is standing.</summary>
        static void Down(ColonyWorld colony, Pawn bandit, Pawn colonist)
        {
        ...
        /// <summary>
        /// Who moves how: the bandit's kind names <see cref="TraverseMode.Bandit"/> while its
        ...
        public void EachKindMovesInItsOwnMode()
        {
            ...
            Assert.That(content.ModeOf(PawnKindIndex.Bandit), Is.EqualTo(TraverseMode.Bandit));
            Assert.That(content.SpeciesOf(PawnKindIndex.Bandit).traverseMode, Is.EqualTo(TraverseMode.Colonist),
                "the bandit is still a person: the mode is its kind's, not its species'");

            Pawn bandit = Spawn(colony, PawnKindIndex.Bandit, Near(colony, 8, 0));
            Assert.That(bandit.OwnMode, Is.EqualTo(TraverseMode.Bandit));
            Assert.That(colony.Pawns.Pawns.All[0].OwnMode, Is.EqualTo(TraverseMode.Colonist));
        }

        /// <summary>
        /// A closed door is a wall to the hog and to the bandit and nobody else; an open one is a
        /// floor to everybody. The bandit wades, climbs ladders and takes stairs as a colonist does.
        /// </summary>
        [Test]
        public void AClosedDoorIsAWallToTheBanditAndTheHogOnly()
        {
            var colony = Bare();
            ...
            var shut = new Dictionary<TraverseMode, bool>
            {
                ...
                [TraverseMode.Animal] = false, [TraverseMode.Bandit] = false,
            };
            ...
            Assert.That(TraverseModes.Swims(TraverseMode.Bandit), Is.True, "a bandit wades as a person does");
            var ladder = new Connector(0, ConnectorKind.Ladder, new[] { 0 }, new[] { Size.LayerStride }, Size);
            Assert.That(TraverseModes.Allows(ladder.ModeMask, TraverseMode.Bandit), Is.True, "a bandit climbs a ladder");
        }

        /// <summary>
        /// The bandit's mode is the colonist's less a closed door, on every link of a real board: the
        /// ruined city, with its thirty-four doors. Every link and portal edge that carries the
        /// colonist carries the bandit, except a link into a closed door, which carries only the
        /// colonist. So a new rule that forgot the bandit — a mask, a hop — fails here.
        /// </summary>
        [Test]
        public void OnEveryLinkTheBanditsModeIsTheColonistsLessAClosedDoor()
        {
            ColonyWorld colony = Golden.City.Build();
            NavGraph nav = colony.Pawns.Nav;
            NavGrid grid = nav.Grid;
            int colonistBit = TraverseModes.Mask(TraverseMode.Colonist), banditBit = TraverseModes.Mask(TraverseMode.Bandit);
            ...
            for (int r = 0; r < nav.RegionCapacity; r++)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    bool colonist = (mask & colonistBit) != 0, bandit = (mask & banditBit) != 0;
                    links++;
                    if (closedDoor && colonist)
                    {
                        doorLinks++;
                        Assert.That(bandit, Is.False, $"link {l} into a closed door carries the bandit");
                    }
                    else
                    {
                        Assert.That(bandit, Is.EqualTo(colonist), $"link {l} ({mask:x2}) treats the bandit unlike a colonist");
                    }
                }
            }
            ...
            for (int c = 0; c < grid.Flags.Length; c++)
                for (int e = nav.FirstPortalEdge(c); e >= 0; e = nav.PortalEdgeNext(e))
                {
                    ...
                    edges++;
                    Assert.That((mask & banditBit) != 0, Is.EqualTo((mask & colonistBit) != 0),
                        $"portal edge {e} from {c} treats the bandit unlike a colonist");
                }
        ...
        /// <summary>
        /// Behind a closed door a colonist cannot be reached by a bandit, and can by a colonist,
        /// who is also free to walk out. The district says so, and so does the step the mover checks.
        /// </summary>
        [Test]
        public void BehindAClosedDoorSheIsUnreachableToABanditAndNotToAColonist()
        {
            ...
            Pawn bandit = Spawn(colony, PawnKindIndex.Bandit, East(inside, 4));
            var ctx = colony.Pawns;

            Assert.That(ctx.Reachable(bandit, inside.Cell, TraverseMode.Bandit), Is.False, "a bandit can reach her through a shut door");
            Assert.That(ctx.Nav.IsLegalStep(East(inside, 2), door, TraverseMode.Bandit), Is.False, "a bandit may step into a shut door");
            Assert.That(new PathFinder(ctx.Nav).FindPath(bandit.Cell, inside.Cell, TraverseMode.Bandit).Status,
                Is.Not.EqualTo(PathStatus.Success), "a bandit found a path through a shut door");
        ...
        /// <summary>
        /// The owner's C6 answer, "kill colonists, destroy base", with the door holding: the bandit
        ...
        /// </summary>
        [Test]
        public void ABanditBreaksTheDoorDownAndThenGoesForHer()
        {
            var colony = Bare();
            ...
            Pawn bandit = Spawn(colony, PawnKindIndex.Bandit, East(colonist, 5));

            TickUntil(colony, () => AttackingBuilding(bandit, handle), 60, "the bandit never went for the door");
            ...
            for (int t = 0; t < 20_000 && colony.Grid.Edifice[door] == handle; t++)
            {
                ...
                Assert.That(bandit.Cell, Is.Not.EqualTo(door), $"tick {t}: the bandit stood in the doorway of a door still standing");
                Assert.That(colony.Doors.IsOpen(door), Is.False, $"tick {t}: the door was opened");
            }
            Assert.That(colony.Grid.Edifice[door], Is.Not.EqualTo(handle), "the bandit never broke the door down");
            ...
            TickUntil(colony, () => bandit.CombatTarget == colonist.Id.Value, 600, "through the door, and it did not turn on her");
        }
        ...
        public void WithTheDoorOpenItGoesStraightIn()
        {
            ...
            TickUntil(colony, () => bandit.CombatTarget == colonist.Id.Value, 60, "through an open door, and it did not go for her");
            TickUntil(colony, () => Melee.InReach(colony.Pawns, bandit, colonist, TraverseMode.Bandit), 600,
        ...
        /// <summary>
        /// Target choice is unchanged (design 33 §16c): the nearest colony building to the bandit,
        ...
        public void ItTakesTheNearestBuildingNotTheDoor()
        {
            ...
            TickUntil(colony, () => AttackingBuilding(bandit, west), 60, "the bandit did not go for the wall nearest it");
        }

        /// <summary>
        /// A bandit knocked back is not knocked into a shut door (<see cref="CombatSystem.KnockbackCell"/>
        ...
        /// </summary>
        [Test]
        public void ABanditIsNotKnockedIntoAShutDoor()
        {
            var colony = Bare(colonists: 2);
            ...
            Pawn bandit = Spawn(colony, PawnKindIndex.Bandit, before);

            Assert.That(CombatSystem.KnockbackCell(colony.Pawns, hitter, bandit), Is.EqualTo(-1),
                "a bandit was knocked into a shut door");

            colony.Pawns.Pawns.Despawn(bandit);
        ...
        /// <summary>
        /// An idle bandit wanders in its own mode. With nobody standing and nothing of the colony's
        ...
        /// </summary>
        [Test]
        public void AnIdleBanditDoesNotWanderThroughAShutDoor()
        {
            var colony = Bare();
            Pawn colonist = colony.Pawns.Pawns.All[0];
            ...
            Pawn bandit = Spawn(colony, PawnKindIndex.Bandit, Size.Index(c.X, c.Z, c.Y));
            Down(colony, bandit, colonist);

            int wanders = 0;
            for (int t = 0; t < 6_000; t++)
            {
                colony.World.Tick();
                if (bandit.CurrentJob?.DefIndex == JobIndex.Wander) wanders++;
                Assert.That(bandit.Cell, Is.Not.EqualTo(door), $"tick {t}: the idle bandit walked into the shut door");
                Assert.That(bandit.CurrentJob?.DefIndex, Is.Not.EqualTo(JobIndex.AttackMelee), $"tick {t}: it attacked something");
            }
        ...
        /// <summary>
        /// With everybody down and only a bed to break, a bandit leaves it alone (design 33 §16):
        /// a downed colony keeps somewhere to be carried to. With a wall beside it too — further off
        /// than the bed — it breaks the wall.
        /// </summary>
        [Test]
        public void ABanditLeavesABedAloneAndBreaksTheWallInstead()
        {
            var colony = Bare();
            Pawn colonist = colony.Pawns.Pawns.All[0];
            ...
            // A bandit that came for nothing (design 33 §17): with nobody standing and only a bed,
            ...
            colony.Pawns.Content.KindMotive[PawnKindIndex.Bandit] = Motive.None;
            ...
            {
                colony.World.Tick();
                Assert.That(bandit.CurrentJob?.DefIndex, Is.Not.EqualTo(JobIndex.AttackMelee), $"tick {t}: it attacked the bed");
            }
            ...
            RaiseAt(colony, wall);
            colony.World.Tick();
            Assert.That(colony.Pawns.Distance(bandit.Cell, bed), Is.LessThan(colony.Pawns.Distance(bandit.Cell, wall)),
                "the control: the bed is the nearer");
            TickUntil(colony, () => AttackingBuilding(bandit, colony.Grid.Edifice[wall]), 600, "it did not go for the wall");
        }

        /// <summary>
        /// The bed is spared from the bandit's choice alone: a player may still order an attack on
        ...
        public void APlayerCanStillOrderAnAttackOnABed()
        {
            ...
            RaiseAt(colony, bed, BuildingHandle.Bed, facing: 1);
            colony.World.Tick();
            Assert.That(BuildingTargets.IsBanditTarget(colony.Grid.Edifice[bed] >= 0
                ? colony.Construction.Edifices.Records[colony.Grid.Edifice[bed]].Def : (ushort)0), Is.False,
                "the control: a bed is not a bandit's target");
            Assert.That(Draft(colony, colonist), Is.EqualTo(IntentRejection.None));
