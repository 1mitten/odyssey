...
namespace Odyssey.Sim.Pawns
{
    ...
    public sealed class MovementSystem : IWorldSystem
    {
        ...
        /// <summary>Jumps over a stream that landed on the far bank (design 46).</summary>
        public int JumpSteps { get; private set; }

        /// <summary>Jumps over a stream that fell short, into the water (design 46 §6).</summary>
        ...
        void Advance(Pawn pawn)
        {
            ...
            while (pawn.HasPath)
            {
                ...
                // Take-off (design 46 §6). The one roll that decides a jump is made the first
        ...
        /// <summary>
        /// Roll a jump over a stream once, at take-off, and record where it lands (design 46 §6).
        ...
        /// <summary>
        /// The chance a jump falls short, per mille (design 46 §6): the base, doubled (by default)
        ...
        /// <summary>
        /// The one roll (design 46 §6), on <see cref="PawnPurpose.Jump"/> keyed by the tick and the
        ...
        int StepCost(int from, int to, TraverseMode mode)
        {
            ...
            // A jump over a one-cell stream (design 46). Without this it fell through to
