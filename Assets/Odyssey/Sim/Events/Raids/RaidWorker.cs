...
{
    /// <summary>
    /// A raid's own parameters (design 50 §8), the nested block design 23 §8 recommended the day
    ...
    {
        // ---- the size, when the caller leaves it to the incident (design 50 §9) ---------------
    ...
    /// <summary>How big a raid is when the caller leaves it to the incident (design 50 §9).</summary>
    ...
    /// <summary>
    /// A raid (design 50): a band of hostiles, made from a mix, that walks on along one edge,
    ...
    ///
    /// <para><b>Refused, not trimmed</b> (design 50 §9): a band that would take the board past
    ...
    public sealed class RaidWorker : IncidentWorker
    {
        ...
        public override bool TryExecute(IncidentContext ctx, in IncidentParms parms)
        {
            ...
            // The Events row: the mix and the size ride in the entry's detail (design 50 §7).
