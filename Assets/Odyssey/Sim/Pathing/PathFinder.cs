...
namespace Odyssey.Sim.Pathing
{
    ...
    public sealed class PathFinder
    {
        ...
        public PathResult FindPath(int start, int goal, TraverseMode mode, PathOptions options)
        {
            ...
            PathStatus status = SearchCells(start, goal, mode, options, constrained, rstamp,
            ...
            {
                status = SearchCells(start, goal, mode, options, false, rstamp, out cost, out int extra);
                cellNodes += extra;
            }
        ...
        PathStatus SearchCells(int start, int goal, TraverseMode mode, PathOptions options,
            bool constrained, int rstamp, out int cost, out int expanded)
        {
            cost = 0;
            expanded = 0;
            ...
            while (_cellHeap.Count > 0)
            {
                ...
                if (x > 0) Relax(c, c - 1, g, goal, mode, stamp, rstamp, constrained);
                if (x + 1 < _sizeX) Relax(c, c + 1, g, goal, mode, stamp, rstamp, constrained);
                ...
                if ((_grid.Flags[c] & NavFlags.Connector) != 0)
                {
                    for (int e = _graph.FirstPortalEdge(c); e != -1; e = _graph.PortalEdgeNext(e))
                    {
                        ...
                        RelaxExplicit(c, n, g + _graph.PortalEdgeCost(e), goal, mode, stamp, rstamp, constrained);
                    }
                }
