...
- All four views are single-layer, roofless cut-aways. The composite never shows two storeys at once. Whether to render the storey above as a ghost outline or hide it entirely is unresolved and is a Lane B question (Going Medieval and Timberborn take different positions). **Still open**, but now judgeable by eye: `docs/reference/mockups/hud-v1.html` implements all three policies — hide, ghost outlines, x-ray — as a live toggle over a layered scene. The provisional recommendation is ghost outlines only, with the layer below dimmed where it shows through holes.
