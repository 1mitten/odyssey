...
- Bottom-centre: a second row of six colonist cards, each with a job or weapon glyph. Duplicates the top bar; one of the two is redundant. Our design must choose one and give the other slot to layer navigation.
...
- Alerts stack on the right edge, newest at top, in a red that reads at a glance even against the night scene. Keep this, but our alerts must say *which layer* the event is on and offer a jump.
...
- All four views are single-layer, roofless cut-aways. The composite never shows two storeys at once. Whether to render the storey above as a ghost outline or hide it entirely is unresolved and is a Lane B question (Going Medieval and Timberborn take different positions).
...
- Adds a UI requirement to the design phase: a layer control and a depth cue that the concept omits. Recorded for `docs/design/06-rendering-and-camera.md`.
