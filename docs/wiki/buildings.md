...
80 entries, 22 without art. Names are what the player sees; the key beside each is the stable identifier — cite it when proposing a change.
...
| **Brazier** | `ui.arch.tool.brazier` | Heat and light, no power, some risk | sheet 03 (camp and crafting), high | M3 |
