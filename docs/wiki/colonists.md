...
69 entries, 51 without art. Names are what the player sees; the key beside each is the stable identifier — cite it when proposing a change.
...
| **Trading** | `ui.status.trading` | Walking to a trader, or negotiating with one while the trade window is open | no art | TR |
