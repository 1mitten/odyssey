...
7. **The frame budget is a pathfinding problem, and it is now the project's top technical risk.** Plain has 3.8 ms of frame left for rendering at a 3× discount and none at 4×. The original explanation — that most of the 65% A-star cost was futile searches for unreachable targets — was tested and falsified: only 14% of budget exhaustions were unreachable, under 1% on a structured map (`docs/design/05-ai-and-jobs.md` §6). The reachability structure is kept for a different, larger reason: a job-giver scan must ask "can this pawn get there?" thousands of times per tick, and answering that with A-star is unaffordable regardless of failure rate — `Reachable` has to be a free integer comparison. The measured win against the tick itself came from the abstract region stage and its heuristic: 2.4× overall, pathfinding down from roughly 65% to roughly 45% of the tick, not to nothing.
