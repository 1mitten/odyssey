...
- **The frame budget is tight.** At a 3× hardware discount, three ticks leave 3.8 ms of a frame for rendering; at 4× there is nothing. This is the project's top technical risk and it is a pathfinding problem: 65% of the tick is A-star. The original explanation — that most of that was futile searches for unreachable targets — was measured and falsified: only 14% of budget exhaustions were unreachable, under 1% on a structured map. Reachability is still answered before pathing because a job-giver scan needs it for free, thousands of times per tick; the measured win against the tick itself came from the abstract region stage and its heuristic instead. `05-ai-and-jobs.md` §6 has the measurement.
