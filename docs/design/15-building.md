...
- **The two slab arts do not share a top surface.** Both catalogue rows carry `baseAtY: 0`, so
  neither is normalised to the cell's floor plane and each sits on its own prefab pivot.
  `SM_Env_Ground_Tile_Half_01` is a thin street tile and `SM_Bld_Base_Floor_Combined_01` a thick
  deck, so a stone cell's walkable surface is drawn *below* its wooden neighbour's — visible in the
  screenshot as a lip of deck edge round every grey plate. **This is the whole of "it landed one
  height below"**: a fraction of a cell, not a layer, and the simulation has them level.
...
**The levelling is not attempted here**, because which of the two meshes moves is a question about
how a deck should look and only the owner can answer it; and because the catalogue is generated by
`PlayScene.cs` under Unity, which was open on this machine at the time.
