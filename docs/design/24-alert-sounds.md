...
| `alert-raid` | 9.54 s | 2 | `ui.alert.raid`, raised while a raid assaults (design 55 §7, 2026-09-25). The source is Pixabay's `low-horn-185556`, trading_nation |
| `alert-raid-arrive` | 18.0 s | 2 | a raid's Events row, when it arrives at the edge (design 55 §7). Pixabay's `war-horn-horror-73771`, baked by `bake_raid.sh` |
