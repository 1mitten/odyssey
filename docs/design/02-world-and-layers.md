...
- **Water: surface water is built, fluid simulation is not** (2026-09-16, ADR 0009). Ponds, streams, marsh fringes and the occasional river are generated, and water is two `TerrainDef` entries rather than a depth field. A channel is cut **one layer down**, so a bank is real geometry and a colonist steps down to wade; deep water is **not** cut deeper, because the bed is the column's surface and a two-layer core would put neighbouring surface cells two layers apart. Deep water needed `CellFlags.ImpassableTerrain`, because neither `solid` (walk across the lake) nor non-solid (walk through it) can say "neither stand in nor stand on". Shallow water costs a third of walking speed through the cost class in `NavGrid`, which had been plumbed and inert since the pathfinder was written.

  Still open here: **water at L−1 as a utility main**, which the strata model still leaves room for and nothing yet uses; and **layer question 10**, the unit of simulation for gas, fire, water and sound. ADR 0009 does not answer it — nothing about this water propagates. A water cell is a terrain, not a fluid.
