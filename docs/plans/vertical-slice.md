...
| Unit | Size | Depends on | Done when |
|---|---|---|---|
| **HT1 The navigation rebuild is local** | M | — | **Done 2026-09-25, `claude/ht1-local-districts`** (`docs/design/05-ai-and-jobs.md` §7). The split was taken first (`NavGraph.RebuildTicks`): at the scale target the districts were 1.00 of 1.80 ms and the portal table 0.42, so all four global passes went, not districts alone. Portal edges and the adjacency are kept as links come and go, in exactly the order the searches walk; districts are repaired from the edit's seeds, with the ends of every built link joined first — the audit's "re-flood the touched component" would have been the full pass, because on a real board the touched component is the board. **Scale target, one mined cell: 1.795 → 0.184 ms**, under the 0.2 target; ruined city 3.454 → 0.263. Every golden, path checksum and region count unchanged. An in-graph oracle (`DerivedTablesDisagree`) checks all four tables after every randomised edit, and each part was withheld and seen to fail. |
...
| **HT10 The enclosure under edits** | M | HT1 | **Found by HT1, 2026-09-25** (design 05 §7g). With the navigation local, a throwaway probe timing every world system on the scale-target lattice, one cell mined a tick, read **Enclosure 8.04 ms, Needs 4.16 ms**, Navigation 2.97, everything else under 0.03; the whole tick is 0.15 ms at rest. The enclosure solve (`28-temperature.md`) arrived after the audit, and `main`'s edit tick on that arm had grown to 63.8 ms. First step, as HT1's: time the enclosure solve by segment and find what it walks per edit; then why Needs costs 4 ms only under edits. Done when the lattice edit tick at the scale target is dominated by no board-scaled system, with the goldens and the temperature tests unchanged. |
