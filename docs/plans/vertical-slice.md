...
| **U40 Colonist select** | M | U37, U39 | Three cards, per-slot reroll and lock. Skills read through the existing `PawnAspect` seam, so `Sim.Contracts` does not change. **Unblocked 2026-09-17:** both dependencies are in, and the screen it hangs off is `MenuScreen.NewGame`. |
