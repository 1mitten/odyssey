...
| **U44 Stairs — built 2026-09-26, not yet played** | M | U43 | Two cells rising 1.5 m each, `ConnectorKind.Stair` with `AllMask`, so a **hauler** can use one. This is what makes an upper storey somewhere a colony can actually build, and U43's hauler exclusion is the argument for it. The two-cell footprint wants a placement rule a ladder did not need. **Built** (`docs/design/63-stairs.md`): `Building_Stair` at handle 14, edifice 24, one record over two cells like the bed; one `Stair` connector from the foot to the cell over the upper half; the ladder's shaft rule two cells long, both sides, built or ordered; `StairTests` (16), including a hauler carrying twenty wood up to a stockpile on the storey above. No golden moved. Eleven defaults wait on the owner (design 63 §10). |
...
| Unit | Size | Depends on | Done when |
|---|---|---|---|
| **HT1 The navigation rebuild is local** | M | — | **The measurement half landed 2026-09-21 with MZ** (`docs/design/28-map-size.md`): `TickBenchmarkTests` has its busy arm (`MineOneCell`, one mined cell a tick, 50 colonists, every offered board) and `NavGraphStatisticsTests.TimePerEdit` has the same figure on a board the generator made — **0.298 ms on Standard, 0.883 on Huge, 1.047 at the scale target**, which corroborates the audit's 0.449 / 1.150 on a slower container. **Two things that change this unit.** Quote the `NavGraphStatisticsTests` figure and not the tick benchmark's: its room lattice carries nine times a generated map's regions, so its edit arm reads an order of magnitude high. And the per-segment split was **deliberately not** taken through `PhaseSink` — `TickSegment` is the tick's seven ordered phases and they sum to the tick, so five nav sub-phases would break that invariant and the margin table with it; take it as counters on `NavGraph` instead, as step one here. What remains is the behaviour change: districts are recomputed only for the components the affected zones touch, and district ids stop being renumbered from zero (nothing compares them for order; they are not saved or hashed). **Done when the edit-tick arm measures under 0.2 ms at the scale target** on the machine that measured 1.19, with every path checksum, every golden and `NavGraphStatisticsTests`' region counts unchanged, and the incremental result tested against a full rebuild as its oracle over random edit sequences — the support solver's pattern. Design section in `docs/design/05-ai-and-jobs.md` beside §6. |
...
| **HT8 Frame time at the scale target** | S | Unity | **Mostly built, unrun.** `FrameTimeTests.TheBoardSizeAgainstTheFrame` (MZ, 2026-09-21) times Standard, Large and Huge with `SeedOrders` on each, reporting `FrameSection.World` and `FrameSection.Doors` and the chunk count; `SeedOrders`, `TimeFrames` and the `FrameSectionMs` split all arrived with the mark-pass work. It is **one test and not three arms on purpose** — a frame number is only comparable with one taken in the same run — and it **holds the colony fixed**, because `PawnPose.Of` is 13.3 ms at 384 pawns and would otherwise be read as a board-size effect. **Run 2026-09-21: Standard 3.18 ms, Large 6.09, Huge 7.82 — over the 5 ms budget, and all of it `FrameSection.World` (2.146 → 5.950) while every other section stays flat.** Draw calls 1,475 → 5,392. **So the one behaviour change this unit was to decide is decided: frustum culling in `ChunkRenderer.Render` goes ahead**, and on a 600 m board seen through a 160 m camera it should be a large win. Two things still left: the 250 × 250 × 40 arm, and an arm that **keeps editing while it times** — `FrameSection.Doors` read 0.000 on every board because `DoorDirector` only rescans when the version moves, so its 922k-cell scan is unmeasured rather than absent. Batching the per-order marks was done 2026-09-20, and the 4.6 us constant that made it look urgent does not apply to a mark plate (§6c.1). |
