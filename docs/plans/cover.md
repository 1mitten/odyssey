...
| **CV4** pass-through | `NavFlags.PassThrough`. `Standing.CanStandAt` as the one owner, asked by every picker listed in §5. The raise guard. The sweep. The audit of the 31 `IsWalkable` callers, written into design 50 §5. | §5 | fast tier per picker; **Long tier**: ten days, a sandbag ring round the start, the sweep never fires |
...
| **CV6b** the art | **Done in code instead, 2026-09-25** (design 50 §7a-bis): `SandbagMesh`, one bag, laid bag by bag by `CoverShape` in running bond, after research `e-12`. The Blender set is not needed unless the owner's eye says otherwise. The barricade was taken out on the same look (§13). | §7a-bis | Unity tiers; the owner's eye |
