...
The design is `docs/design/50-raids.md`. Branch `claude/sharp-lamport-8q5u4h`, one commit per
unit, one PR. The interview (2026-09-25) is recorded in design 50 §2.

| Unit | What | Gate |
|---|---|---|
| **R1** | Design 50, this plan, follow-on notes in designs 23 §8 and 43 §9 | docs only |
...
| **R8** | `TickBenchmarkTests` with 200 raiders; index only what the numbers show; `PawnCeiling` 200 → 400 with `PawnCeilingTests` in the same commit | measurement in design 50 §11a |
