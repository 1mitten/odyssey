...
**Checkpoints 2 and 3 together** (`docs/plans/combat.md`): health and melee against a bandit
...
debug menu is **backtick**; its **Spawn** tab has *Spawn bandit* and one row per weapon.
...
| The bandit | none | *Spawn bandit* puts one near the camera **holding a machete**, under a red diamond; it hunts the nearest colonist **standing** and ignores the downed | §1, §6A.6, §6D |
| Right-click with drafted colonists selected | always a move | on an **animal or a bandit**, an attack by every drafted colonist selected; **Ctrl** on a colonist attacks her; on the ground, still a move | §2f, §6C |
...
| Downed | — | lies where she fell, needs paused; a colonist heals **only in a bed**, an animal wherever it lies, a bandit never | §6A.3–4 |
...
| A bandit's pane | a colonist's, with a Draft button | no face, no tabs, no commands; its line says what it is doing, e.g. *Fighting* | §6C |
| Melee | not a skill | a live skill on the Skills tab, trained by every swing | §5a |
| A second colonist hitting a bandit | it dropped the swing it was winding up and seldom turned on her | it keeps fighting the colonist in front of it and goes for the hitter when that one is down; a bandit still chasing somebody turns on whoever hits it | §6A.6, §6F |
...
|---|---|---|
| Spawn a bandit beside two colonists and **do not draft** | it walks to the nearer one and swings; she stops work and fights back; numbers float off both | the bandit stands idle; she carries on working while being hit; the two circle each other and never land a blow |
...
| Fight until **somebody goes down** | she drops to the ground; her bar empties to red; *Downed* floats and lingers; the bandit turns to the next colonist still standing | she stays upright; the bandit keeps beating a body on the ground; the bar vanishes |
| **Finish a downed bandit**: draft a colonist, right-click the body | she strikes until it dies; *Dead*; the body stays lying — it does not get up to fall again; clicking it says *Corpse of a bandit*; the machete lies beside it | the body vanishes, or stands and falls over; the machete is gone; a click on the body selects nothing |
| Two colonists, **undrafted**, beside one bandit | it keeps swinging at the one in front of it; when she goes down it turns on the other, not on whoever is nearest | it stops landing blows while both hit it; it ignores the one still hitting it; it flips between them every blow |
...
| Draft two, select both, right-click a bandit | the ring closes in and flashes, then stays faint until the bandit is down | noticed only as it lands (0.2 s too quick), the flash reads as a glitch, or the ring is lost on grass or at night |
...
| A bandit's weapon | in the hand | always in the hand, unchanged | §8b |
...
| Bandits overlapping | — | step round colonists and each other like colonists (animals stay out of it) | §8c |
...
| Spawn a bandit by an undrafted, armed colonist | she draws when it is 2 tiles away and fights back | she fights with the weapon still at her hip |
...
| Four bandits round two colonists | everyone on their own tile, bandits stepping round each other | two on one tile, or bandits merging |
...
| Attacking the dead | an attacker kept its attack on a dead pawn for a tick | an attack ends the tick its target dies; nothing targets the dead, bandits included | §9e |
...
| Debug menu | 280 px, one list | 460 px; Spawn tab under headings: Colonists (with **Arm every colonist**), Hostiles (with **Spawn 3 bandits**), Animals, Weapons, Items | §9i |
...
| Test | Look for | A wrong answer looks like |
|---|---|---|
| Debug, Spawn tab, **Arm every colonist**, then **Spawn 3 bandits** | every colonist has a weapon at the hip; three bandits on three tiles | someone bare-handed, or bandits stacked |
