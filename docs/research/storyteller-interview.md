...
**Read next:**
- `docs/design/68-storyteller.md`: the design these answers decide.
...
- **Strength-based budgets invite the inverse meta: disarm before a raid.** If raid size follows
  what the colony holds at the instant it fires, stowing weapons shrinks raids. Design 68 §4c
...
- **Our threat budget cannot be checked against the reference's numbers.** Its curves are built on
  wealth. Ours start invented and are tuned by the soak (design 68 §9), which is the instrument.
...
- **Every number in design 68** is invented until the Long-tier soak has run.
