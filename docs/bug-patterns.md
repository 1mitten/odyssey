...
### P14 — One version number for many caches, so every edit invalidates all of them
...
(`docs/design/28-nobody-in-a-wall.md`): a detour that keeps passers-by out of the cell, a guard that
...
P14 above: one global version, 45 chunks re-meshed for one cell, 12.53 ms in that frame. That is
