...
| **PS** prisoners | **Built 2026-09-26 (P1–P12), reviewed twice, merged with `main`, through both Unity tiers; not yet played** — branch `claude/prisoner-bed-assignment-98afc0` (review worktree `D:\dev\odyssey-prisoners`), design `docs/design/60-prisoners.md` (**§15 is as built, §16 the second review**; renumbered 58 → 59 → 60 as cracks and then world generation took them first; **save format 12**, since world generation took 11), interview `docs/research/prisoner-interview.md`, research `a-20`, plan `docs/plans/prisoners.md`. Marking a bed *for prisoners* makes its enclosed room a **cell** (a prison bed with no room **shackles**); prisoners walk in `TraverseMode.Bandit`, so a door holds them unless somebody holds it open. **In** by capture (right-click a downed enemy: a menu, *Capture* / *Finish off*, replacing the instant execution), a warden's capture mark, **surrender** (a raider under 30 % may yield if a cell is free) and **arrest** (a right-click row on a standing colonist while nobody selected is drafted, so a drafted right-click stays a move; dim with its reason when it would be refused — the owner moved it off the pane's header, which it left 17 px for her name). **Warden** work: capture, feeding, recruiting, letting go. A **visible willingness bar** with its hours and blockers (`Recruitment.Factors`), a **visible escape risk** with its reasons, × 1/√n (`EscapeRisk.Odds`), an escapee **bashes the door**, runs for the edge, and is a prisoner again if downed. Fates: Hold, Recruit, Release, Exile (the pane's mode row opens the four to choose from); an arrested colonist released goes back to work. **`Kind` is never rewritten** — custody (`Pawn.Custody`, hash bits 28–29) and one owner of sides (`Allegiance`); **one bed rule** (`BedRules`). The only golden bake was P3 (Social, format 10 → 12 as merged, Warden). Debug → Spawn: Imprison, Free, **Break out**. |
...
| **Prisoners: prison beds and cells, custody, `Allegiance`, `BedRules`, recruitment, escape** | `docs/design/60-prisoners.md` |
