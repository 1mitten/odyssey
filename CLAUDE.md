...
| **AN** animals | **Ground and interview 2026-09-21, nothing built** — `docs/design/31-animals.md`. The owner wants a pig (their own model) in the world, moving, harmless, clickable with food and rest readable, interface later, performance first. Grounded: a pawn is a heap object with virtual decision points and a documented `PawnRegistry.Adopt` seam for a subclass, **not** a table; `TraverseMode.Animal` and its districts already exist and are tested; the roster and the selection box assume a person; and the per-pawn crowd scan in `PawnPose.Of` (13.3 ms at 384 pawns) is quadratic in the snapshot, so it is the prerequisite for 300 animals. Recommended: `Animal : Pawn` with an `AnimalKindDef`, the think tree a property of the pawn, `PawnView.Kind` on the contract, save format 9 with the kind first in the record, one figure director and one cap. Seven units `AN1`–`AN7`; **eleven questions in §5 wait on the owner and no unit starts before they are answered.** |
...
| Animals, the pawn-kind seam, the species Def, wildlife population | `docs/design/31-animals.md` |
