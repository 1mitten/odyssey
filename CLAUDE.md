...
| **RC** ranged combat | **Approved and built 2026-09-25, not yet played** — branch `claude/ranged-combat` (stacked on PR #220, the design), worktree `D:\code\odyssey-ranged`; design `docs/design/47-ranged-combat.md` (§10 lists where the build departed from it), plan `docs/plans/ranged-combat.md`. A pistol (*Pistol*, Battle Royale's `SM_Wep_Pistol_Heavy_01`) on the melee line's seams. **Simulation**: a **Shooting skill** (save format **9 → 10**, an older colonist dealt it once on load, goldens re-baked once in R0 with the colony probe identical on all three boards); `LineOfSight` — a three-dimensional integer supercover, symmetric, with the lenient corner so a colonist fires down off her own terrace; `Projectiles` — every bullet saved, hashed only while one flies, landed on the tick decided at the shot against whoever is on its line then (a blocker, the target on a shot aimed true, a bystander past a 5 m dead zone, the ground; **a downed pawn never takes a stray**, and a shot that missed never takes its own target); `AttackRangedJobDriver` — aim paced by condition not skill, the aim broken with the cooldown given back when the line closes, no minimum range, no shot from water; **`CombatJobs` owns which attack a pawn makes**; fire at will for the drafted and for pistol bandits; a gun-holder's order on a building is refused until R6. Debug: *Spawn pistol*, *Spawn pistol bandit*, and the Arm row deals it. **Played once 2026-09-25** (*"really decent"*): the accuracy raised, a miss now carries on past its target into the ground, and a hit lands on the target wherever it stands (design 47 §10a). **Weapon quality** for every weapon, the beds' tiers and roll scaling damage and hit chance, rolled when a weapon is made — a find or a bandit's gear until crafting exists (§11). **Presentation**: the pistol fitted by its grip, the sword pack's cross-draw from the left hip, low ready, a two-handed aim with the gun as the truth and both hands IK'd to it, a spring recoil and the slide cycling; the tracer, flash, dust and the owner's gunshot (near and far, split at 70 m). **Owed**: R5 (the ten-day soak with gunmen, the fifty-shooter benchmark), R6 (buildings under fire), P4 (the frame with gunfire), a player build, and the playtest. |
