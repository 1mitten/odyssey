...
| **SY** storyteller | **Designed 2026-09-26; its screens built the same day, not yet played; the simulation (ST1–ST7) waits for approval** — branch `claude/sweet-cerf-wzfxgs`, design `docs/design/59-storyteller.md`, plan `docs/plans/storyteller.md`, interview `docs/research/storyteller-interview.md`, research `a-11-storyteller-pacing.md`. Three pacing shapes (Steady cycle, Calm cycle, Chaotic bag) as Defs on one generator kit, names later. The threat budget is **colony strength** (standing colonists' health × weapon × skill, read as a remembered peak), **never wealth and never fortifications**, × a day ramp × **visible** tension (a five-band gauge by the clock) × difficulty (six rungs + Custom). Both choices are **colony state** changed by intent, picked on the setup page and changeable in Settings. `IncidentParms.Points` stays a raid size; the storyteller sends Auto. Units ST1–ST7; the joiner (ST7) makes population intent live. The Claude Design brief for the picker, the Settings rows and the gauge is `docs/reference/mockups/storyteller-brief.md`. **The screens are Claude Design's mockups 25a–25h** (design 59 §12): the storytellers are **Jacob, Trent and Kano**; a Story block of three cards, a seven-rung ladder and an always-drawn Custom block on New game (compact below the page's own 1,692 px); Pausing and Story in Settings > Gameplay; a 16 px tension gauge on the clock, whose date lost its season to make room; a raid pauses a running world. The choice is **carried in `StoryDirector` and read by nothing** until ST1, and the gauge's band is a debug preview until ST3. Jacob and Trent left the colonist name pool, replaced in place by Jonah and Wade. |
