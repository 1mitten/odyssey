...
| **CM** combat | **C1 built and played 2026-09-23** (drafting, T, moves, the diamond and the four hours all work; the owner then asked for the run and a deeper, translucent red, both in), **in review; C2–C7 designed** — `docs/design/33-combat.md`, plan `docs/plans/combat.md`, stacked on WL (animals are targets). The owner's MVP, interviewed in five rounds: draft, move, melee against animals, colonists, buildings and a debug marauder, four pack weapons, HP pool with downed and dead, rescue to a bed. **C1 is the draft**: **T** or the pane's button drafts the selection (R stays slice-up); a drafted colonist holds, neither eats nor sleeps, and is let go by a break, by exhaustion or after four quiet hours; a right-click with no tool armed moves every selected drafted colonist, spread over neighbouring cells. **An order given mid-step lands the step first** (`Pawn.FinishingStepTo`, saved and hashed) — the snap the job expiry was taught to avoid. Draft state is its own save section (`odyssey.combat`, no format bump) and is hashed only while set; all six goldens moved for two job defs and the probe diffs clean. `JobSystem.Load` now accepts saves with fewer job defs. The Synty Sword Combat clips are unpacked into the shared `Assets/Synty` for C2. |
