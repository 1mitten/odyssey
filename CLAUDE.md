...
| **CK** cooking | **Designed 2026-09-25, nothing built** — branch `claude/cooking`, worktree `D:\code\odyssey-cooking`, design `docs/design/48-cooking.md`, plan `docs/plans/cooking.md`, interview `docs/research/cooking-interview.md`, research `a-18`, `a-19`, `e-10`, `e-11`. The owner's chain — hunt a hog, butcher where it fell, raw meat to a powered fridge, cook on a galley or campfire from bills with skill deciding burnt meals, eat at a table with plate, knife and fork — in four playable units, **kitchen first** (K1 kitchen, K2 cold store and rot, K3 hunt and butcher, K4 dining). Every moment is animated by computed poses over measured points; the chair sit is decided by one `SeatProbe` contact sheet before K4. **Waits on the owner**: plan approval, design 48 §13 (mood numbers, two names, the rotten haul, the unpowered fridge), and a moment with no editor open to import the POLYGON Shops pack as `PolygonShops/` only (it ships PolygonGeneric with identical GUIDs). Handle numbers are allocated at build time: #225 and #222 claim the next ones. |
