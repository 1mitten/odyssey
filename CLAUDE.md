...
| **HE** health | **Built 2026-09-25, in review — PR 1mitten/odyssey#213**, branch `claude/relaxed-heisenberg-zxy63b`, design `docs/design/43-health.md` (§14 is as built), plan `docs/plans/health.md`, brief `docs/reference/mockups/health-tab-brief.md`. **Neither Unity tier has run** — built in a cloud container; the Health tab's view was type-checked only against stubs. A person's body is **six regions over the unchanged pool**; blows and falls land on regions by coverage, a limb passes its excess to the torso, injuries **merge by region and kind** (eighteen at most). Pain, consciousness, moving and manipulation are derived; **pain shock at 800 ‰** downs at 64 points; a vital region at nought downs, it does not kill (design 33 §3 still holds). Untended cuts bleed and kill through `Kill`; any tend stops every bleed. `CombatSystem.Hurt` is **the one owner of damage**. The Doctor column and the Medicine skill are live; the doctor fetches a medkit if she can reach one and tends from a free side. Falls do 15 × n^1.5 (a collapse from one layer; any drop past one). The Health tab is two columns of seven on the Skills grid; condition and weapon moved to the header line. Alerts for untended injuries and no medicine; debug Hurt, Heal, Kill and Give medkits; right-click **Tend**. Save format unchanged; goldens moved once, as more zeros, probe clean. **The fight's balance moved** (43 §14b): the colony now wins every gate seed and no colonist dies; bandits bleed out. |
...
- **Fast tier** (`scripts/test-fast.sh`, ~55 s, no Unity): **1,521 Sim + 1,103 Hud**
  (2026-09-25, health H1–H6); Long tier **48** in about three minutes, a third of it the
