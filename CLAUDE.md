...
| **AL** the Almanac | **Built 2026-09-26, not yet played** — branch `claude/almanac-refresh`, worktree `D:\code\odyssey-almanac`, design `docs/design/64-almanac.md`. Rewritten against the code: the mock-up's steel, pines, traits and silver are gone; 110 pages in 19 categories, keyed by registry key (name and index line are the registry's; `emit_labels.py` describes the Almanac's namespaces), links by key, the info button by the key the pane draws, and a button that finds the thing or opens its tab. **`AlmanacCatalogueTests` fails on anything the HUD can show without a page**, and `AlmanacFactsTests` holds 93 quoted numbers to the Def XML. Thirty false registry descriptions corrected. Icons: the owner's art on 16 pages, a line icon on the rest; the route to the other 91 is design 64 §4. |
