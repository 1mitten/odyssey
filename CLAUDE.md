...
| **RC** ranged combat | **Designed 2026-09-25, nothing built — PR #220**, branch `claude/beautiful-cannon-pldakk` — design `docs/design/44-ranged-combat.md`, plan `docs/plans/ranged-combat.md`, interview `docs/research/ranged-interview.md`, research `a-10-ranged-combat`, `a-10-projectile-path`, `c-3d-shot-line`, `d-21-projectile-rendering`, `d-22-procedural-aim-and-recoil`, `e-10-gun-animation-packs`. A basic pistol on the melee line's own seams: a ranged block on the weapon Def, a **Shooting skill** (save format 9 → 10, one golden re-bake in R0), a **three-dimensional line of sight** (an integer supercover between cell centres, symmetric by construction; rock, walls, closed doors and the slab of the upper cell block; a lenient corner so a colonist fires down off her own terrace), a saved and hashed bullet with a fire tick and an impact tick, a miss scattered in the target's layer by how bad the shot was, probabilistic interception with a 5 m dead zone (**a downed pawn never takes a stray**, so the combat gate's invariant holds), fire at will for the drafted, a procedural two-handed aim with four empty clip rows as the pack seam, and one instanced tracer bucket charged to Overlays. Units R0–R7, H1, P1–P4. **The next hard stop is the owner's approval of design 44 — its §8 lists seven recommendations to confirm — and no gameplay code is written before it.** |
...
| **Guns, bullets, line of sight in three dimensions, the aim stance, the tracer** | `docs/design/44-ranged-combat.md` (designed 2026-09-25, not built) |
