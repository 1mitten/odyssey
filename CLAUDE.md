...
| **EX** expeditions | **Designed 2026-09-26, nothing built; the plan waits for approval** — branch `claude/compassionate-hypatia-cz8reg`, design **64** (`docs/design/64-expeditions.md`), plan `docs/plans/expeditions.md`, interview `docs/research/expeditions-interview.md`, research `a-13-travel-and-caravans`, `b-away-play`, `b-voyages-and-situations`. Colonists travel the planet as an **expedition**: a token on the hexes, about 6 h each, camping at night, with 0–2 road choices later. They arrive at a **place** whose authored **situation** is cast from its seed and played on a real **Standard** board. **Home keeps running in full on one clock** (every board's tick equals the campaign's). **One screen, no split**: board tabs, an away strip, alerts labelled by place that pause with Go. The board is discarded when the last colonist leaves and the place remembers its state. Sea is a later layer on the same planet. First slice EX0–EX11, a round trip to a salvage cache; EX0 is a spike moving a pawn between two worlds, because moving a pawn (EX3) is the riskiest unit. |
...
| **Expeditions: travel on the planet, places, situations, more than one board, switching boards, the away strip** | `docs/design/64-expeditions.md` |
