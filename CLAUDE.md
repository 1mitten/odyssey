...
| **HE** health | **Designed 2026-09-25, nothing built** — `docs/design/43-health.md`, interview `docs/research/health-interview.md`, plan `docs/plans/health.md`, brief `docs/reference/mockups/health-tab-brief.md` (with Claude Design; H5 waits on it). **Six body regions over the unchanged pool** — the pool of 100 that dies at −50 is already `a-02`'s "150 damage is death", so nothing saved, hashed or drawn about it moves. Afflictions (wound, bruise, fracture) **merge by region and kind, so a person carries at most eighteen**; pain, consciousness, moving and manipulation are derived; **pain shock at 800 ‰** downs at about 64 points where today it is 100, and the combat soak's downs-per-raid before and after is the number that decides it (§3). Any tend stops every bleed; `Work_Doctor` lights the Work tab's already-drawn column; Medicine goes live on the Melee pattern with **no format bump**; fall damage 15 × n^1.5 through the one damage owner extracted from `ApplySwing`, so a fatal fall is mourned. Animals stay on the pool. Goldens move once, at H3. The tab is briefed to **fit the Skills tab's grid** (seven 19-px rows, two columns) unless the designer names a height. Deferred and recorded (§11): lethal cold and hunger, infection, alerts, right-click Tend, the debug rows. |
...
- **A hit-point pool, but no body** (rewritten 2026-09-25; "no health model" was stale since combat
  C2): fall damage has a number and now a design (43 §7) and nothing applies it, a colonist rides
  a collapsing floor down unharmed, and the debug menu has no hurt, heal or kill. Design 43 is
  written; nothing of it is built.
