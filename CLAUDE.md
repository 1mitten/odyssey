...
| **TR** trading | **Designed 2026-09-26, nothing built. The plan waits for approval**: branch `claude/wonderful-clarke-kkd9pq`, design `docs/design/57-trading.md`, plan `docs/plans/trading.md`, interview `docs/research/trading-interview.md`. A debug-menu event brings one trader in from an edge to the hearth for about a day. A selected colonist is sent with right-click *Trade*, and a pausing B10 ledger opens. **Gold is a real item** (`ui.res.credit` renamed). Prices are `marketValue` ×1.4/×0.6 with one owner (`TradePricing`); the purse is limited and the stock rolled; goods are sold out of stores and bought goods set down beside the trader; the deal is atomic. It needs a **neutral side** (`Faction.Visitor`, its own mind, a `PawnFlags` bit, widened to `ushort`). Harm: accidental → the trader leaves, deliberate → hostile. Units T1–T7. |
