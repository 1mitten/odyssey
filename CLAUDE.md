...
| **TE** temperature | **Built, reviewed and fixed 2026-09-21 — PR #164, ready for the playtest.** The review (`docs/design/28-temperature.md` §12) found nine faults by probe test and §12a records the fixes: the severity slope was twenty times its own comments, the enclosure sweep stopped a layer short and cost 5–6× a solve, a floor taken out never told the enclosure, a re-sealed room kept a season-old temperature and a loaded one did not, a shared slab was charged to the sky, body heat truncated to nothing, a colonist's ambient did not survive a load. Fifteen regression tests; goldens re-baked and **measured** component by component to be the hash seeing two more fields and caverns converging, with the colony doing nothing different.** M4's core pulled forward (design 28, the model `a-06` recommended): every enclosed room is one integer scalar, one pass per 120 ticks over cached surfaces — per-room, never per-cell. Seasons and the day's own swing come from a `ClimateDef` (Wash benign, Glare warm, **Rime kills**), the ground damps the seasonal swing by depth so a cellar lags and a deep mine holds the annual mean, and warm air climbs a stairwell at **4:1** over cold falling down it — the whole of buoyancy as one asymmetric conductance. Enclosure grew room identity keyed by min cell index and the **shaft rule** (a hole into the room above still counts as roofed — the round trip caught the loaded-and-played worlds disagreeing about which rooms existed, which is why the sweep now runs to fixed point), `WeatherOffsetC` is the seam a cold snap adds to, and a **campfire** (`Building_Campfire`, 3 wood) pushes energy so a broom cupboard is an oven. Temperature moves mood (banded, situational), sleep (×0.9/0.75/0.55), work (×0.7 outside 8–35 °C), `TemperatureSeverity` (signed ±1000, starvation's exact shape, save format **8 → 9**) and crop growth (the 6/42/58 response). The pane says **how warm a tile is** on the click, tinted; the clock says the outdoor reading. Goldens re-baked twice — the second time because the convergent sweep changed which caverns exist. `docs/design/28-temperature.md`. |
