...
| **RC** ranged combat | **Designed 2026-09-25 and reviewed the same day; nothing built — PR #220**, branch `claude/beautiful-cannon-pldakk`, worktree `D:\code\odyssey-ranged` — design `docs/design/47-ranged-combat.md` (**renumbered from 44**, which `main` gave the selection highlight), plan `docs/plans/ranged-combat.md`, interview `docs/research/ranged-interview.md`, research `a-10-ranged-combat`, `a-10-projectile-path`, `c-3d-shot-line`, `d-21-projectile-rendering`, `d-22-procedural-aim-and-recoil`, `e-10-gun-animation-packs`. A basic pistol on the melee line's own seams: a **Shooting skill** (save format 9 → 10, one golden re-bake in R0), a **three-dimensional line of sight** (integer supercover, symmetric, a lenient corner so a colonist fires down off her own terrace), a saved and hashed bullet with a fire tick and an impact tick, scatter by how bad the shot was, interception with a 5 m dead zone (**a downed pawn never takes a stray**), fire at will for the drafted, a procedural two-handed aim, one instanced tracer bucket charged to Overlays. **The prop is POLYGON Battle Royale's `SM_Wep_Pistol_Heavy_01`** (owner's pick, measured: 0.046 × 0.21 × 0.39 m, five parts on one material with the melee weapons' shader). **The owner's gunshot is baked and committed** (`tools/audio/bake_gunshot.sh`: an outdoor space, three near and two far takes split at 70 m, −14 LUFS) and plays nowhere until P3. **The review (design 47 §9) corrected** the purpose salts (three taken), the aim's rate, blood, the clip rows and the tracer's reuse claims, and **added** the animation and flight rules the draft left open (§2e, §4b's state table, §4c). **Handles 23 / 11 / 6 are raced by #213, #184 and #223**: R0 takes what is free when it merges. **The next hard stop is the owner's approval of design 47 — §8 lists eleven recommendations — and no gameplay code is written before it.** |
| **CV** cover | **Designed 2026-09-25, nothing built** — branch `claude/cool-darwin-akh02q` (documents only), design `docs/design/50-cover.md`, plan `docs/plans/cover.md`, interview `docs/research/cover-interview.md` (27 answers), research `a-10-cover`. The reference's partial cover — the eight cells round the target, angle bands with the diagonal ×1.75, the shooter's distance to the cover, noisy-OR, and a defeated shot redirected into the cover, which wears it down — **plus a height term of our own graded by the shot's angle of descent**, all in integers. **Sandbags** (5 stone, placeholder) and a **barricade** (wood or stone; art follows the material) are 55 %, dragged as a line, joined, crossed at +150 / +250 and **never stood on** (`Standing.CanStandAt`, one owner); walls, rock and doors are 75 %, trees 25 %, bushes 15 %, furniture 25–50 %. Bandits pick covered firing spots, drafted colonists hold, and both crouch behind low cover (pose only, derived, unhashed). **Stacks on `claude/ranged-combat`, whose code is not on `main`: merge ranged first.** Embrasure, lean and repair are later units. **The next hard stop is the owner's approval of design 50 — §10 lists five points.** |
...
| **Guns, bullets, line of sight in three dimensions, the aim stance, the tracer** | `docs/design/47-ranged-combat.md` (designed 2026-09-25, not built) |
