...
| **SG** the stair gait | **Built 2026-09-24, EditMode tier green (3,283), awaiting its first play — branch `claude/stair-gait`, design `docs/design/41-stair-gait.md`.** The third attempt at making a bank crossing read as climbing rather than as an escalator, from the owner's plan reviewed against the code: a smoothed sawtooth **added to the ramp** (cosine treads, C1, zero at every tread boundary), phased to the climbing *half* of the step — the plan's whole-step curve would have bobbed the flat approach, which is the parabola's fault from the other side. Banked descents step down through the same surface-following branch (gravity kept for sheer edges); a knee lift fades in over exactly the band where `Footing.Correction` fades out; the plan's body bob (no walk-cycle phase exists to drive it) and forward lean (banks are not in the relief field `Footing` reads) were reviewed out and recorded as seams. One owner for "is this a banked climb" (`StairGait.IsClimbing`, asked by pose and footing both). Budgets: worst per-mille 12 mm up / 24 mm lowering and worst frame 38 mm against the 50 mm teleport ceiling; `BankFootingTests` runs stair-on unchanged, and the two `HopArcTests` claims that are the glide's own ("on the ramp and nowhere else", "no rhythm") now pin the **stair-off** glide, as strict as ever. Everything is a live tunable (`StairGait.Enabled` returns the glide in one switch). |
...
| The stair gait, bank crossings drawn as steps, its tunables | `docs/design/41-stair-gait.md` |
