...
| **BU** the Pig Butcher | **Built 2026-09-26, played once, four levels added — PR #248** — branch `claude/pig-butcher`, worktree `D:\code\odyssey-butcher`, design `docs/design/62-pig-butcher.md`, research `e-16`, `b-large-enemies-and-knockback`. POLYGON Fantasy Rivals imported (its own folder only). A hostile **kind 6** in **one cell drawn 3.64 m tall** (measured, scale 2.0), bulky by `TraverseMode.Animal` (no ladder, breaks doors), 500 hit points on `Health_Brute` (pain shock at 320), never stunned or knocked back, its cleaver its species' natural attack. Its swing **sweeps the front arc of three** (flanks on their own dice, the facing in spare `PendingSwing` bits) and **flings** three landed blows in four two cells straight back: a **slam** into a wall or a body, a **fall** off a ledge, then 300 ticks of knockback immunity (`CombatSection` layout 6, hash bit 28). Drawn as itself (`OwnPaint`, its own box, its own portrait via `PortraitStudio.ForKind`), the cleaver in the fist (a new prop is parked at the hip; the natural one is put back every frame), a red **telegraph** on the arc during the wind-up, *Slam* floaters. **Four levels** (kinds 6–9, the pack's colourways 01_A–D, 4.4 to 5.6 m, pool 500 to 1,400, the king flings three cells) on abstract `ButcherBase` / `BruteBody`. **Debug spawn only**; no raid mix until played. As tuned even level 1 beats four bats every time (§4b). No golden moved. |
...
| **FA** forest animals | **FA1 built 2026-09-26, in review** — PR (number to follow), branch `claude/forest-animals`, worktree `D:\code\odyssey-forest`, **stacked on #248 (merge that first)**; plan `docs/plans/forest-animals.md`, designs 64–67, interview `docs/research/forest-animals-interview.md`. Nine species at kinds 10–18 (Verge rabbit, Hedgerow deer, Ash fox, Gutter raccoon, Rubble skunk, Thicket boar, Mire moose, Ridge wolf, Quarry bear) from Synty's SIMPLE Forest Animals under `Assets/Synty/SimpleForestAnimal` — licensed and gitignored; it installs to an unignored folder, so `tools/synty/unpack.py --remap` moves it. Doe/stag and cow/bull are derived from the seed (`Pawn.Form`, never saved); drawn at life × the colonists' 1.42; 32/48/64/80 animals by board, the hog moved to the city. **The Almanac shows every animal's whole behaviour, each fact live or *Not yet in the game*** (plan §1a): FA2 and FA3 flip a fact in the commit that builds it, and `TheLiveLinesAreExactlyTheseToday` holds them to it. FA2 (temperament, predators with the owner's **kill-window exception to design 33**, the far form) and FA3 (signatures, hours) are designed and wait for FA1's play. |
...
| **Animal temperament: the five rungs, noticing a person, warnings, herds, a colonist giving way** | `docs/design/64-animal-temperament.md` |
| **Predators: hunger, prey, the kept kill, man-eaters, the kill window** | `docs/design/65-predators.md` |
| **Forest animals' art: the import, forms and colourways, scale, the far form** | `docs/design/66-forest-animal-art.md` (§14 as built) |
| **Animal signatures and hours: spray, rut, grazing, raids, winter away** | `docs/design/67-animal-signatures.md` |
| **The forest animals' order, seams, wildlife table and the Almanac's live/not-yet rule** | `docs/plans/forest-animals.md` |
