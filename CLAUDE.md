...
| **GE** gear | **G3, the kit, built 2026-09-26, not yet played** — branch `claude/gear-kit`, stacked on G1, design **54**: two belt slots of medical supplies (5) or rations (3), *Take into kit* from the right-click menu or *Pick from stores*, Remove/Drop, and **Use**; a doctor treats from her own kit first, spent in place and never lifted; a ration only when no other food can be reached; no golden moved. **G1, the Gear tab, built 2026-09-25, merged with `main` and through both Unity tiers 2026-09-26, played (owner: "good enough for now")** — branch `claude/vigilant-bardeen-8idplc`, design **47**. Claude Design's paper doll on the colonist pane (the tab body is **244 on every tab** now); the **hand is real** — `OrderUnequip` (Unequip leaves it to the haulers, Drop forbids it) and *Pick from stores* sending the context menu's own `OrderEquip` — and everything else is the debug menu's **Preview full kit**, interface state only. Interviewed the same day: Twenty-two owner answers (`docs/research/gear-interview.md`): one weapon; worn **Head, Face, Body, Armour, Back** with warmth, armour, rain and mood; quality, no wear; the jumpsuit is the empty Body slot; a **kit** of belt 2 + pack 4 slots with a size class, not weight; a **per-trip cap** per commodity on heavy loads; **loadouts** on the Assign tab; a Strip order; no tools, no drag and drop. The Claude Design prompt for the end state is `docs/reference/mockups/gear-brief.md`; units G1–G8 are `docs/plans/gear.md`. Next: research `a-19`, then the design doc. |
...
| **Gear: the weapon slot, the kit, worn slots, loadouts, the trip cap; the Gear tab and its preview** | `docs/design/47-gear-tab.md`, `docs/research/gear-interview.md`, `docs/plans/gear.md`, design 33 §9d, design 29 §9 |
