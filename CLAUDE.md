...
| **HA** home area | **Designed 2026-09-25, nothing built; waiting on the owner's approval and the Claude Design brief** — branch `claude/sleepy-cannon-9d0evw`, design **43**, interview `docs/research/home-area-interview.md`, research `a-18`, brief `docs/reference/mockups/home-area-brief.md`, units H1–H5 in `docs/plans/home-area.md`. Home is **derived**: everything the colony placed (buildings, floors, sites, zones, lines; not felling or mining marks) grown by **five cells, square**, per layer with one layer of margin, rebuilt lazily and never saved or hashed. A **Home** switch goes under Power on the views strip. Each colonist is **Anywhere** or **Home**, set on a new **Assign** tab on **F4** beside the combat Response: at Home she takes no work outside, walks home when idle, and a draft overrides it. **An empty home restricts nobody**, and a new colony has none (no beds, no stockpile). |
