...
| **CV** cover | **Built 2026-09-25, not yet compiled in Unity or played** — branch `claude/cool-darwin-akh02q` (carries `claude/ranged-combat` merged in: **merge ranged first**), design `docs/design/50-cover.md` (§12 as built), plan `docs/plans/cover.md`, interview `docs/research/cover-interview.md`, research `a-10-cover`. The reference's partial cover in integers — the eight cells round the target, angle bands with the diagonal ×1.75, the shooter's distance to the cover, noisy-OR, a defeated shot fired **into** the cover so it wears — plus **a descent term of our own** (low cover gone by 35°, tall by 60°). `Cover.BaseAt` is the one owner of what a cell is worth. **Sandbags** (handle 13, edifice 23, always 5 stone via `fixedStuff`) and a **barricade** (14, 24, wood or stone) are 55 % low cover, dragged as a joined line, crossed at +150 / +250 (`NavFlags.PassThrough`, one price for both searches) and **never stood on** (`Standing.CanStandAt`; a sweep steps off anybody at rest on one, read from saved state). A stray crossing cover is caught at half its value past the dead zone; destroyed cover leaves a quarter. Bandits and undrafted defenders pick a covered spot for the first 240 ticks of an attack (`CoverPosition`); drafted colonists hold. Crouch behind cover (a derived aspect, unhashed), a *Cover* floater, a hit-point bar over **every** struck building, and a hit-chance readout at the pointer (`QueryShot` → `ShotReportView`). Placeholder boxes for the art. **No golden moved.** Embrasure, lean and repair are later units. |
...
| **Cover, sandbags, the barricade, pass-through cells, crouching behind cover** | `docs/design/50-cover.md` |
