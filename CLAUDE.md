...
- **Fast tier** (`scripts/test-fast.sh`, ~20 s, no Unity): **918 Sim + 609 Hud** (2026-09-21,
  `claude/huge-map` merged with a main carrying the almanac, storage zones and the skills work; the decoration work adds no fast-tier test, because it is all Presentation);
  Long tier **34**, up from 23 because the per-board measurement arms all carry `Category("Long")`.
...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-21 on
  `claude/huge-map` **after merging main, the decoration measurement, the surround cut and the
  overlay repair**: EditMode **2,320 total, 2,297 passed, 0 failed**; PlayMode **94 total,
  89 passed, 0 failed** (PlayMode last run before the overlay repair, which is Presentation-only
  and compile-proved by the EditMode run after it). The
  three new EditMode ones are `SurroundCostTests` — the surround charged to itself rather than to
  the board, and the guard that the variant count is what the wood's batch count is made of; the
  two new PlayMode ones are `FrameTimeTests.TheDecorationAgainstTheFrame` and
  `TheSurroundSectorSweep`. On the merge
  alone it was EditMode 2,317 / 2,294 / 0 and PlayMode 92 / 87 / 0, and before the merge
  **2,285 / 2,262 / 0**. The run on `main` the same day, PR #139 merged with
  ...
  two `PortraitLightingTests`, the two `AvatarSheetTests` and `FigureCapTests` each ignored itself
  and said so in its skip reason. Re-run with the art restored it is 86, matching `main` exactly.
  **Read the skip reasons before reading a lower *passed* count as a regression** — `failed` is the
  number that matters and it was 0 both times.
  The run before both, on `claude/mark-pass-batching` after merging main: EditMode **2,228 total,
