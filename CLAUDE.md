...
| **CD** the draw | **Designed 2026-09-24, nothing built** — branch `claude/character-draw`, worktree `D:\code\odyssey-draw`, design `docs/design/41-the-draw.md`, Claude Design brief `41-the-draw-brief.md`. Colonist select gains **traits** (a small real set, seven seams, one aggregator, the Def shaped for full traits later) and **two modes, chosen one or the other**: **Standard**, where everyone is dealt the same total (exactly 12 levels over the five live skills, one major and one minor passion, one good and one bad trait), and **Gamble**, one pull per colonist, locked, from long-tailed tables to 15 (simulated: +32% on average, 9% duds, 6.8% with a 12+ skill Standard never deals). Gamble is a ten-reel **fruit machine** after *Alternate Reality: The Dungeon*'s stat strip: one STOP, the colonist drawn at the press, the reels cascading left to right to land on it, a hot reel teasing longer. Standard replaces the roll every colonist uses, so **every golden will move with behaviour**, not just the hash. Units CD1–CD6 in §8; **waiting for the owner to read the design** before CD1. |
