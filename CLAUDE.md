...
| **SY** storyteller | **ST1–ST4 built 2026-09-26, with its screens; merged with `main` and reviewed the same day, ready for its first play — PR #254** — branch `claude/sweet-cerf-wzfxgs`, worktree `D:\code\odyssey-story`, design `docs/design/68-storyteller.md` (§12 the screens, **§13 the simulation**), plan `docs/plans/storyteller.md`, interview `docs/research/storyteller-interview.md`, research `a-11-storyteller-pacing.md`. **Jacob, Trent and Kano** (Claude Design's mockups 25a–25h): three pacing shapes on one generator kit (`Storytellers.xml`) — a cycle, a random bag with a drought rule, a long calm cycle — firing the incidents that exist, gates read for the first time, grace holding back big threats only. **Raids are sized by colony strength** (standing colonists' health × weapon × skill, a remembered peak falling 4 ‰ an hour), **never wealth and never fortifications**, × a day ramp × threat % × tension; a debug Auto raid is the same raid. **Tension** (400–1500, five bands) falls on a colonist's death or down and recovers on quiet days, scaled by adaptation. The choice is **colony state**: `odyssey.storyteller`, hashed only while set (**no golden moved**), changed by `SetStoryteller`/`SetDifficulty`; the difficulty ladder is written once, in the Hud's `StoryCatalogue`, and the intent carries the levers. `StoryDirector` reads `StorytellerView` and submits the intents; an old save says *No storyteller: choose one in Settings* once. Harness (200 seeds): big threats a season 2.95 / 3.20 / 1.66; the Long soak agrees (3 / 4 / 1). **The review (design 68 §13h)** fixed a Trent who went silent for good on Peaceful (the drought forced every roll into a big threat and threw it away), a second colony whose first raids never paused the game (the Events panel kept the last colony's ids), a difficulty that looked refused on an old save and then came true, and the intents' clock; **renumbered 59 → 68** on the merge (world generation took 59). **The portraits are in**: the owner's illustrated cards, cut to three heads that fill the card's 64 px tile (`Resources/Odyssey/Storytellers/`, full cards in `art-source/storytellers/`). **Owed**: `BulletinView.Category` (pause on every big threat), ST7 the joiner, a season played on each. Jacob and Trent left the colonist name pool, replaced in place by Jonah and Wade. |
...
| **The storyteller: the pacer, the strength budget, tension, difficulty** | `docs/design/68-storyteller.md` |
...
- **The storyteller is built but unplayed** (design 68 §13, `claude/sweet-cerf-wzfxgs`); on `main` nothing fires an event but the debug menu (owner's call, 2026-09-20). The
