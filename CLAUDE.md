...
| **GE** gear | **G1, the Gear tab, built 2026-09-25, not yet played or compiled in Unity** — branch `claude/vigilant-bardeen-8idplc`, design **47**. Claude Design's paper doll on the colonist pane (the tab body is **244 on every tab** now); the **hand is real** — `OrderUnequip` (Unequip leaves it to the haulers, Drop forbids it) and *Pick from stores* sending the context menu's own `OrderEquip` — and everything else is the debug menu's **Preview full kit**, interface state only. Interviewed the same day: Twenty-two owner answers (`docs/research/gear-interview.md`): one weapon; worn **Head, Face, Body, Armour, Back** with warmth, armour, rain and mood; quality, no wear; the jumpsuit is the empty Body slot; a **kit** of belt 2 + pack 4 slots with a size class, not weight; a **per-trip cap** per commodity on heavy loads; **loadouts** on the Assign tab; a Strip order; no tools, no drag and drop. The Claude Design prompt for the end state is `docs/reference/mockups/gear-brief.md`; units G1–G8 are `docs/plans/gear.md`. Next: research `a-19`, then the design doc. |
