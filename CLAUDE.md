...
| **WG** world generation | **Designed 2026-09-26, nothing built — waiting on the owner's approval and on Claude Design** — branch `claude/sharp-euler-a6xtci`, design `docs/design/57-world-generation.md`, interview `docs/research/world-generation-interview.md`, research `a-13`, brief `docs/reference/mockups/world-map-brief.md`, plan `docs/plans/world-generation.md`. A seeded planet of 64 × 32 hex tiles wrapping east–west; the player picks a site on a new **World** screen before the setup page, and the site sets the board's **climate and seasons** (latitude scales the swing; one reference latitude reproduces today's curve exactly) and its **hills** (Flat, Rolling = today, Hilly, Mountainous at 24 layers). **Only Meadow can be settled** until the next art pack; the other nine biomes are shown. **A request with no site builds exactly today's board**, so no golden may move. Save format 10 → 11 (the header carries the tile). The biome and terrain names are in the registry as proposals for veto. Factions, rivers, roads and coast are recorded seams. |
