...
| **FT** faces and talking | **Built 2026-09-26, not yet played — PR #247**, branch `claude/face-expressions` (worktree `D:\code\odyssey-faces`), design **59**, research `e-15`. The mechanism only (owner: *"we are just looking at animation/mechanism now"*): every live colonist **blinks**; a face can hold six **expressions** (Neutral, Raised, Alarmed, Stern, Sceptical, Tired) on the rig's one `Eyes` and one `Eyebrows` bone, written **absolutely from rest** by `FaceRig`; and two colonists can **talk** — turns, phrases and pauses, head beats with brow emphasis for the speaker, slow nods for the listener, each looking at the other through a new gaze tier (`GazePriority.Conversation`). **No mouth** (the owned lips cannot open without tearing the head) and **nothing starts a conversation or picks an expression** but the debug menu's new **Faces** tab. Logic is engine-free in `Odyssey.Hud` (`FaceMotion`, `Conversation`); the whole pass for 64 figures is 19 us a frame. `FaceLiveTests` (PlayMode) proves it on a live colonist, and fails with the pass switched off. Nothing in a cell, a save or the hash. |
