...
Read this first, then `docs/code-map.md` (where things live, the tick and the frame as built, and how to add one of each), `docs/brief.md` (the governing brief) and `docs/research/INDEX.md` (what is known so far). Update the **Current status** section whenever it changes, and **append the reasoning to `docs/journal.md`** rather than growing this file — that is what the journal is for.
...
| **RV** architecture review | **Done 2026-09-26** — `docs/audit/2026-09-26-architecture-review.md`, `docs/code-map.md` (the subsystem map every session should read second) and `docs/plans/refactoring.md` (R0–R9, each sequenced behind the open PRs on its file; it supersedes HT5). The finding: every monolith the 09-19 audit named is 1.8–3× larger a week later, and nothing made the growth a decision. **The size ratchet is in the content gates from this commit** (`tools/ci/size_ratchet.py`): 38 files over 800 lines are listed with a ceiling in `tools/ci/size-ceilings.json`, and a PR that grows one past it raises the number in the open. No C# changed — the container has no dotnet SDK, so nothing unprovable was touched. |
| **HT** hardening | **Audited 2026-09-19; audit and plan merged as PR #136. HT1 in review (PR #211); HT5 superseded by `docs/plans/refactoring.md` (2026-09-26); the rest not started.** `docs/audit/2026-09-19-baseline.md` is the baseline audit — scalability measured at the scale target, the monoliths, the process, and everything not yet addressed — and `docs/plans/vertical-slice.md` §HT is the ordered list of hardening units that came out of it. The first finding with a number: a tick that edits one cell at 250 × 250 × 40 costs 1.19 ms against 0.065 ms at rest, all of it `NavGraph.Rebuild` recomputing every district. **Phase gate: the plan is written and waits for approval; HT1 is the one unit started.** |
...
- **Content gates — there are four, and the third is the one that gets forgotten:**
  ...
  before a content commit. **The fourth is the size ratchet** (2026-09-26): `python3
  tools/ci/size_ratchet.py` fails on any production file grown past its ceiling in
  `tools/ci/size-ceilings.json`; a PR that must grow one raises the number in the same diff and
  says why. `docs/code-map.md` says where the new code goes instead. The icon gate turned CI red on 2026-09-21 over a hand-written
