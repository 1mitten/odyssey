...
- **Phase 2 (research): wave 1 complete 2026-09-15** — twelve files delivered (Lane A: pawns, jobs, building/roofs, mapgen, tick model · Lane B: Going Medieval · Lane C: Cataclysm DDA · Lane E: all four Synty-fit files · Lane F: repo prior art, which found a full *Ramble* checkout at `D:\code\ramble`); one-line results per file in `docs/research/INDEX.md`. Still to run: A2/A5–A11/A13/A14/A16, remaining Lane B/C items, Lane D spikes (D1 benchmark narrowed to two-way per Phase 1 Q6). **Interface/UI research is owned by a separate agent session — do not duplicate it here.**
- **Cell size: FIXED at 2.5 × 2.5 × 3.0 m** (`docs/adr/0002-cell-size-and-layer-model.md`).
- **Target: the playable MVP** (vertical slice, M0→M3), fast-tracked per `docs/research/phase1-answers.md` Q8–Q9: slice-critical research only, slice-scoped Phase 3 docs, approval stop before Phase 4 intact. Next unit of work: the D1 benchmark (`docs/research/d-01-architecture.md`, spec ready) → ADR 0005 → Phase 3 docs. Graphics bar: close to the concept renders (`d-03-rendering.md`). A look-check scene exists: `Assets/Scenes/Spikes/VisualBlock.unity` (regenerate via *Odyssey → Spikes*).
