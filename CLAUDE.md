...
| **CM** combat | **C1 merged (PR #176). C2 (health and melee) and C3 (weapons) built 2026-09-23 by four parallel lanes and integrated on `claude/combat-c2` (`D:\code\odyssey-combat`) — awaiting the owner's playtest of both checkpoints; hand-over `docs/plans/combat-c2-handover.md`, no PR yet.** **Played 2026-09-23 ("mostly pretty decent"); the round after it is on `claude/combat-c2-polish` (`D:\code\odyssey-combat-drawn`)**: a context menu to equip, a lock-on ring on an attack target, attackers on distinct sides, and the blood seam (blood itself is the next unit), design 33 §7. **Rounds two and three (2026-09-24) are on `claude/combat-c2-r3`**: the drawn and sheathed weapon at the hip, a steady deeper health bar, one no-shared-tile guard for every fighter, hit reactions, criticals and knockback settled at the swing's start, the dead never targeted, health on the colonist cards, the swing, slice and thud sounds, spawns that spread, and a wider debug menu with grouped spawns and *Arm every colonist*; design 33 §8–§9, hand-over `docs/plans/combat-c2-handover.md`. Hit points with downed at 0 and dead at −50 %, corpses, a debug-spawned marauder holding a machete, right-click attack and equip, the Sword Combat clips with computed fallbacks, health bars and floating words, the Health tab, four weapons drawn in the hand and lying on the ground. **No golden moved after the contracts step** (probe-diffed); a fight in view costs 2.29 ms against 2.06 at peace in one run. **A fight is silent** (five sound ids, no clips), **an unordered fight ends in downs, never deaths**, and a downed colonist cannot heal until rescue (C4). `docs/design/33-combat.md` — §6A–§6D the lanes, §6E the integration; plan `docs/plans/combat.md`. |
