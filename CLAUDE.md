...
| **MD** medical supplies | **Built 2026-09-24, not yet played — branch `claude/medical-supplies`** (`docs/design/37-medical-supplies.md`). The medkit key became **Medical supplies**, stacked 10 and drawn as the Battle Royale first-aid case; the playtest kit carries 6, and a **Medical drop** event brings 4–8. **Doctor** (an emergency work type) and **Medicine** (speed only) are live. A doctor treats a colonist lying still — downed where she fell, or in bed — with one unit (+40, never past 80 %), or a bare dressing (+10) with none; a quarter-day cooldown keeps supplies from standing in for rest. The badly hurt (under 50 %) go to bed as patients until 80 %; with nobody to come, a colonist treats herself for half, three times as slowly, never past 60 %. Rescue (C4) is still a stub, which is why the downed are treated where they lie (§3, option A). Save: combat section layout 4, no game-wide bump. Goldens re-baked, probe clean against `main`. |
...
| **CM** combat | **Done: C1–C6 and every playtest round after them on `main` (PRs #176, #180, #182, #194); C7, the gate, in review — PR #204, `claude/combat-c7`, `docs/milestones/combat-report.md`.** Draft and move, hit points (downed at 0, dead at −50 %), four weapons, blood, rescue to a bed, friendly fire, buildings as targets, doors that hold bandits, thieves that leave, and a colonist's response (fight back, defend, flee). **An unordered fight ends in downs, never deaths.** The gate: ten days on three seeds without hostiles and with seven raids, every invariant held hourly, a lockstep twin hashing the same every hour, and a save mid-raid resuming the same — which is what found the one fault, a step lost across a load in the attack driver (design 33 §21c). A fight costs 0.02–0.09 ms a tick with fifty colonists and ten bandits on the scale target and on Huge. **Open, the owner's:** the combat playtests (C4 onwards unplayed), right-click on your own ladder, door or bed, drawing a damaged building, the Thoughts tab, kidnap, and whether four armed colonists should lose to three bandits. `docs/design/33-combat.md`, plan `docs/plans/combat.md`. **The marauder is the bandit since 2026-09-24** (`claude/bandits`, design 42, in review): renamed everywhere but the journal (kind index 3 unchanged, so no save or hash moved), a crowbar or a bat dealt by a hash of the pawn, and **dressed**: a person rolled as a colonist, then a welding helmet, a red vest and black trousers laid over them, with a name of their own on the pane and a masked portrait. |
...
| **FR** the frog | **Built 2026-09-25, not yet played** — branch `claude/frog`, design 30 §8. The culvert frog, kind 4: the owner's `Frog.fbx` (Quaternius, no walk clip) **hops** on its looping Jump clip, with the drawn position paced to the clip's measured flight (`hopGait`, `HopSurge`) so it sits, leaps and lands rather than gliding. **A bank habitat** (`WaterBank.Near`, one owner): the meadow seeds frogs in threes to fives on the banks of its water, every leg ends within three cells of water (`SpeciesDef.bankRadius`), a stray walks back, arrivals come in only where water meets the edge; meadow density 15 → 27 so hogs and rats keep their share. 0.87 m and an emerald skin of our own (`AnimalImport.Paints`), and a group's hops turned apart (`divergeRadius`), from the owner's two asks of 2026-09-26. Does not swim (a sixth traverse mode is a flood per rebuild); stays out in the rain (`ignoresRain`). Played board golden re-baked, probe clean. |
...
| **MF** the Meadow overhaul | **Designed 2026-09-24, nothing built** — branch `claude/meadow-overhaul`, worktree `D:\code\odyssey-meadow`, design `docs/design/38-meadow-overhaul.md`, interview `docs/research/meadow-interview.md`, research `e-09`, `d-16`–`d-18`. Lush Meadow grass on every tile through our own foliage shader, all-Meadow trees as more sim species, and **a smooth skin drawn over the unchanged simulation layers** in place of the terraces, with ~8 layers of hills. Instanced LOD, no occlusion culling. Ultra 60 fps at 4K; Low 60 fps on an RTX 3050/3060 laptop. **M1 measured 2026-09-24** (design 38 §13, `FrameTimeTests.TheGrassAgainstTheFrame`): grass costs **1.1–1.3 ms at 3840 × 2160** on the RTX 5070 Ti shipped and 0.8–1.9 at full cover, over three runs — the cost is having grass, not how much — so lush on every tile is affordable at today's clump size. The opaque queue was cheaper all three times (0.2–2.5 ms), which makes a rendering-layer mask for the outline the lever to weigh in M3. Research had predicted six draws per instance; for grass it is one (no shadows, queue 2501 outside the prepass), so **depth priming is dropped**. The owner's GPU reading at 4K agrees: tufts off 6–7 ms, on ~7 with spikes to 8 while moving. **M2 built the same day** (§14): instanced levels of detail — one bucket per placement draws every level of a Meadow prefab — **off by default**, because the pack's switch heights put every tuft on screen at its crudest card from this camera; M4/M5 turn them on per art with `LodBias`. **M3 built** (§16): the Meadow grass is drawn by our own `Odyssey/Foliage` — wind on the game clock, grass clearing round items and marks, a lighter spring grade — and is 0.5–0.7 ms *cheaper* at 4K than the pack's shader. **M6 (presets) built 2026-09-24** on `claude/meadow-m6-presets`: Quality Low/Medium/High/Ultra/Custom, a Grass ladder replacing the tufts toggle, grass distance, grass shadows — and stored graphics preferences now reach a new session, which they never had. **The look pass, dressing (design 38 §17, branch `claude/meadow-look-dressing`)**: Meadow trees (several per species, uninked, shadows from a coarse proxy), bushes, tall-grass stands, wildflowers and stones drawn as scenery by noise patches, the art's own colours read at runtime; teal tufts fixed; about 2 ms at 4K on Standard High. **M9, the ground skin, built 2026-09-24** (§20, `claude/meadow-skin`): terraces drawn as ramps over the unchanged simulation (no golden moved), stream banks sloping into the water, an apron to the surround with no line. **Grass at distance (§21)**: bushes never fade; grass thins with distance at every rung by a rank the CPU and shader share (the far-zoom cost was the grass layer itself — mostly the dressing's draw calls — not Full's density); the GPU-driven dressing path is the recommended next lever. **The scenery is GPU-driven** (§22): tufts, grass stands, flowers, ground cover and bushes are drawn from GPU buffers with a compute cull — picture-exact at every framing, 4–5 ms less at 4K zoomed out, and regathered per chunk (0.16 ms a dig on Huge); trees and stones stay on the chunk path. **Trees grouped and simpler far away** (§23): tree calls down 80–96%, near picture unchanged. **M7 measured 2026-09-24** (`28-map-size.md` §11): ±4 relief gives no cliffs; 16 layers leaves no rock under the valleys, 20 gives four for +25% cells, 24 gives eight for +50% (recommended: depth costs the frame nothing, only memory); the relief itself costs ~600 draw calls, which M9's skin must win back — the owner picks. **Shorelines and water** (§24, `claude/meadow-shorelines`): the bank and bed are one height field crossing the water line inside the bank (no staircase, no sim change), marsh and depth are painted from a board field, and the water is the reference's murky teal — draw calls unchanged. **M5 + M13, the scenery made real** (design 45, `claude/meadow-nature` then `claude/meadow-forage` stacked on it, 2026-09-25, not yet played): trees are four species in the simulation (birch, meadow, fruit, giant — wood and work from `WildPlants.xml`, the art follows the id, a felled tree topples); bushes and berry bushes are edifices walked through at +50 and cleared by *Chop and clear*; a ripe berry bush is picked by the new **Harvest** order and regrows in three days; loose stones and mushrooms are items the map lays; the dressing draws none of them. Goldens measured with `GoldenColonyProbe`; the 4K frame flat after the actor pass learned to cull off-screen things. **Meshing re-measured 2026-09-25** (§26, `claude/mesh-cost`): a chunk is 0.29–0.33 ms on Standard timed round `Mesh` itself (0.43 was the frame's delta shared out), a quarter of it was the ground relief recomputed on every re-mesh — now remembered on the half-cell lattice, bit for bit, 23–30 % off — and the meshing budget is **2 ms as well as eleven chunks**, because the worst frame draining a whole-board re-mesh was 8.6–12 ms at eleven. |
| **SW** the settings window | **Built 2026-09-24, not yet played** — branch `claude/settings-frame`, design **39**. One fixed 1240 x 720 frame, centred, the same on every tab: a rail of five tabs with Save / Save as / Load / Quit / Exit once at its foot, a title band, equal columns, a footer with *Reset \<tab\> to defaults*. The empty square in front of every row was `IconBadge`'s placeholder and is gone; Detail is switches that say On/Off; every mark is a drawn SVG path (`SvgPath`, `PathGlyph`) and every string ASCII. **It is a modal now** (scrim behind it). New: per-tab reset (`SettingsDirector.ResetTab`), Backspace clears a listening key slot, and a keyboard path (Tab, arrows, Enter/Space, one drawn focus ring, the game's keys gated while a control has focus). The leave prompt is restyled, focus on Cancel. **The title screen is a 560 px dock on the left** (design **40**): the Strata mark and wordmark, four buttons in the rail's icons and inks, New game focused, Settings opening over it, and Exit game asking through the leave prompt's no-colony form rather than arming. |
| **WD** walls down | **Built and played twice 2026-09-24; ready to merge** — PR #197, branch `claude/walls-down`, worktree `D:\code\odyssey-walls-down`, design `docs/design/42-walls-down.md`, research `b-walls-down-cutaway.md`. Walls, windows, doors and pillars drop to a **0.75 m stump** on every drawn layer and the **stacked** storeys above the slice are hidden (an upper storey; a house on a higher terrace and the landscape stay), so a player can see who is indoors. **Played once**: with the walls down, a lower terrace is ground, not underground, so nothing above it is x-rayed (§3a); the "R / F" label is gone. **On by default**, remembered per player (`GraphicsOption.WallsDown`), switched by **H**, the drawn icon under the depth rail's "R / F", or Settings; **overruled in build mode** (palette open, or Build/Deconstruct armed). `WallsView.Lowered` is evaluated once a frame into `SliceSettings.wallsLowered`, and every pass — renderer, picker, marks, door leaves, colonists, items, corpses, fire, combat marks, blood, sites — asks the slice. **No re-mesh**: `ChunkBatch` holds `Walls` and `Stumps` beside `Body`/`Roof` and the renderer picks one; walls are no longer in `Body`. |
| **WE** weather | **Built 2026-09-25, ready to merge — PR #203**, branch `claude/rain-look`, worktree `D:\code\odyssey-rain`; design `docs/design/43-weather.md` (merged in, with #190/#202/#208), interviews `docs/research/rain-look-interview.md`, research `d-20-rain-rendering`. **The sky** (`WeatherSystem`, Order 35): Clear, Cloudy, Rain and Storm rolled by season from `Weather.xml`, spells of hours blended over two, the temperature's `WeatherOffsetC` finally written, saved (`odyssey.weather`, no bump) and hashed — goldens re-baked, **measured**: the colony probe differs from `main` in mood alone (−200 a board). **The look**: GPU rain in two draw calls (`Odyssey/Rain`), splashes on roofs and rings on water, dry under roofs and canopies (`SkyHeightMap`), wet ground in the meadow and grass shaders, a screen streak layer zoomed out, and the owner's rule that **rain keeps the colour of a clear day** — only Cloudy and Storm drain it (`Overcast` cover vs gloom). The debug menu's **Weather** tab commands the sky; the clock shows it as a glyph. **`weather-world` built 2026-09-25, in review — PR #219**, branch `claude/weather-world`, worktree `D:\code\odyssey-weather-world` (design 43 §6a, §7a). **One shelter rule** (`SkyColumns`, `Sim/World`): a pure column function the rain texture now calls too, held cell by cell to the render mirror on the played board (`SkyAgreementTests`), recomputed for the columns the chunk grid says an edit touched — **so every colony has a chunk grid now, a headless one included**. Rain slows whoever stands under the sky (`Pawn.WeatherPerMille`, ×0.90 at a downpour, a rate and never a path price), waters exposed crops (+25 % at full), and sends animals past 400 per mille to the nearest cover (`AnimalShelterThinkNode`). **The rain is heard**: the owner's light and heavy recordings, baked to seamless equal-loudness loops and weighted by `RainMix` per weather type, with the birds stepping back. Only the city golden moved, measured half by half. **The pace is on screen** (`claude/pace-readout`, 2026-09-25, not yet played): *Pace 90% · in the rain* under the colonist's activity line, its tooltip naming every factor that is not 1,000 (rolled, condition, rain, drafted/running), from four new pawn aspects that multiply back to the move rate exactly; an animal making for cover reads *Sheltering*, derived at publish time rather than a job def (design 17 §5a, 43 §6a). No golden moved. **Owed**: rain on a roof. Owner still to pick: wet ground richer or gloss only; and whether the shelter rule should be rebuilt on `SkyLanding` (PR #219's departure 1). |
| **BD** ambient birds | **Played 2026-09-25 and ready to merge — PR #230** (owner: *"superb"*), worktree `D:\code\odyssey-birds`, branch `claude/wizardly-ride-wely6j`, design **50**, research `d-23`, `e-12`, interview `birds-interview.md`, sketch `docs/reference/mockups/birds/`. Rook flocks and a buzzard over the board, **drawn only**: never in a cell, a save, the hash or the picker. Built in code (28–32 flat triangles, the flap in `Odyssey/Bird`), one instanced call a species; **grow with zoom** (life size at 25 m, ×1.75 at 140 m); rooks perch on drawn crowns and roofs (`BirdPerches`, the sky column rule, `TreeArt.CrownOf`), scatter from a walker within 8 m, roost at a rookery 19:00–06:00, stay down in a storm; the buzzard is out 08:00–17:30 in clear or cloudy weather. The flock is engine-free (`BirdSky`, 27 fast-tier tests); `FrameSection.Birds` is its cost: **0.018–0.032 ms, and the frame cannot tell them apart from off** at 640 x 480 or 4K on Standard or Huge (`TheBirdsAgainstTheFrame`, design 50 §8a). |
| **CD** clouds | **Built 2026-09-26, in review — PR #251** (merge #250, the crack and shard shaders' inclusion, first) — branch `claude/clouds`, worktree `D:\code\odyssey-clouds`, design **63**. The Meadow demo's two cloud rings on the pack's own `Clouds` shader graph, round the camera: a low band under a clear sky standing up into banks as the weather clouds over (cover is the rings' **height** — the shader is opaque and its "Cloud Strength" is a wobble), turning on game time with the wind, coloured from the graded daylight; **gone by night** (sink and dissolve 19:30–21:00, back 05:00–06:30, nothing submitted between), rain heavier with its colour kept, a storm a slate deck darker than the sky (owner, 2026-09-26, §4c–§4d). Seen in First Person; the colony camera sees no sky, so **nothing is submitted there** (the highest ray against the ring's lowest point). Two `RenderMesh` calls when seen, no shadows, queue 2450; the licensed material is copied, never written. The pack's scattering (lilac at noon) and fog (overshoots in our haze, pure blue at night) are off; the haze is in the colours. `FrameSection.Clouds` 0.002–0.010 ms; **about 0.03 ms of GPU at 4K**, measured in a development player by `PlayerBench -odyssey-bench-clouds`. The branch also carries the two-line shader-inclusion fix without which no player could be built from `main` (Crack, Shard). Nothing in a cell, a save or the hash. |
| **JP** jumping a stream | **Built 2026-09-25, played once, three faults fixed; EditMode 3,683 / 0 failed** — branch `claude/funny-allen-2qipcn`, PR #212, design `docs/design/46-jumping.md`. First play: it took off in the water (the shoreline from `main` slopes the bank under it; the lip is now the last dry ground, §7) and no clip played (empty catalogue references, now linked). The same branch carries **the draft's marks drawn through what is in front of them** (design 33 §23) and **a swim stroke sound, one per arm, heard only close** (design 20 §9). A person on one bank of a **one-cell** stream jumps to the other at walking pace (`MoveCost.Jump` 200, against 290 to drop in and hop out) instead of swimming it; wider water is still waded, deep water and trenches are not jumped, animals never jump. `NavGraph.IsJumpAcross` owns the rule and `NavGraph.JumpCost` the price (guarded by `HopPriceHasOneOwnerTests`); the region graph has a `LinkKind.Jump`. **A jump can fall short** — one roll at take-off, 30 per mille, doubled carrying, divided by condition — and lands in the water at the jump's price; `Pawn.JumpLanding` is saved in `CombatSection` layout 4, hashed while set, kept across a load and an order given mid-air. Drawn by `JumpArc` through `PawnPose.Of`, the Base Locomotion walking jump and land clips in the combat action slot; a splash is named (`SoundIds.Splash`) and unsourced. Debug menu > Cheats > *Jumps always fail*. **Only `PlayedBoard.Simulated` moved**; the probe against `main` differs only in routes. **The jump rows were added to `ModuleCatalogue.asset` by hand with empty clip references: rebuild the catalogue on the owner's machine.** |
| **HA** home area | **Played 2026-09-25 and ready to merge — PR #214** (owner: *"it all works"*; reviewed, merged with `main` twice, through both Unity tiers), branch `claude/sleepy-cannon-9d0evw` (review worktree `D:\code\odyssey-home`), design **43**, interview `docs/research/home-area-interview.md`, research `a-18`, brief `docs/reference/mockups/home-area-brief.md`, plan `docs/plans/home-area.md`. Home is **derived and centred on the hearth**: everything the colony placed (buildings, floors, sites, zones, lines; not felling or mining marks) grown by **five cells, square**, per layer with one layer of margin, and **only the piece joined to the hearth** — the first campfire raised, moved by *Make this the hearth*, gone when it comes down. Rebuilt lazily, never saved or hashed. Each colonist is **Anywhere** or **Home** (`SetPawnArea`): at Home she takes no work outside (`PawnContext.MayWork`, the gate inside `Reachable`; the fight asks `CanTravel`), eats outside only when starving, walks home when idle, and a draft overrides it. **No hearth, no home, nobody restricted.** The interface is **Claude Design's specification** (design 43 §5–§6, with the owner's five rulings where it disagreed with the shipped HUD): a **Home** switch under Power (and a live Menu row — the overlay rows are now live exactly for `HudViews.Keys`) draws a 0.25 m **edge, no wash**, at 70 % on the active layer and 30 % below, two meshes and **at most two draw calls**, parting the grass along it, with a **house over the hearth**; the **Assign tab on F4** (536 wide, a row per colonist in roster order, Area and Response cells that cycle, Home and Flee in amber, twelve a page, stays open when a name is pressed, the inspect pane waiting until it closes); a campfire's pane is **wide**, with a *Hearth* line or a *Make this the hearth* button. Only standable border cells are published (`WatchHome`), and `HomeVersion` moves exactly when they change. No golden moved. **The review (journal, 2026-09-25) fixed five faults and two compile errors**: the walk home searched only three layers, so a colonist two down a quarry stood there for good; the Home view drew nothing on its second switch-on; the inspect pane opened over Assign; the hearth alerts went stale; a store at an outpost stopped a Home colonist's haul. And `main` had taken **bit 26** of the pawn hash word for jumping, so **the area is bit 27**. Unity: EditMode 3,848 / 3,814 / 0 failed, PlayMode 150 / 138 / 0; the Home view costs **+0.07 ms and 2 draw calls** at 640 × 480. |
| **PB** polish batch | **Built 2026-09-25, in review — branch `claude/polish-batch`.** Six owner reports. Nobody sleeps in a tree's tile (`JobSystem.FreeSpot`). Nothing lands in one: the rule is in `ColonyItems.CellHasSpace`, so every drop, grant, refund and falling load obeys it. Idlers and sleepers take a tile no other pawn holds or is heading to (`PawnRegistry.IsClaimedByOther`), and the fire grew a second ring. Anyone standing still on a shared tile is drawn apart (`PawnPose.StandApart`, 0.7 m). A roster click selects and a double click closes in to 14 m. The colonist pane is at 85%. **Trees fade for the selection only**, with a Graphics switch for everyone that starts off, reversing design 38 §17c. Two goldens moved and were measured: positions only. |
| **AB** ambient butterflies | **Built 2026-09-26, played once (day *"superb"*, night reworked), draft PR #234** (PlayMode and the contact sheet owed) — branch `claude/ambient-butterflies`, worktree `D:\code\odyssey-butterflies`, design `docs/design/52-ambient-butterflies.md`. Procedural butterflies over open grass near the camera: an engine-free state machine in `Odyssey.Hud` (`ButterflyMeadow`, the birds' shape) behind a habitat seam the render mirror answers, one structured buffer and **one `RenderPrimitives` call**. Wings are shader tables, flapped ~10 Hz with a 42% downstroke; they land on flowers, bask, scatter from colonists within 2.5 m, thin with cloud, leave in rain, none in Rime. **At night each butterfly lights up in its own hue** (cyan, magenta, amber, violet, colour-blind checked) and is drawn to the full zoom; the first build's halos and pools of light were taken out on the owner's first look (*"not those big glowing saucers"*, design 52 §5a), so it is **one call day or night** and nothing reads the depth texture. **Real seconds, not game time**, or speed 3 strobes. Graphics → Detail → **Butterflies** Off / Few / Many / Swarm (0/80/200/500), owned by the presets. Nothing in a cell, a save or the hash. |
| **WK** waking into the world | **Played 2026-09-26 (owner: *"perfect"*) and ready to merge after PR #240** — PR #241, branch `claude/nice-einstein-cwicw1`, stacked on PR #240 `claude/load-curtain` (a load had no cover at all; merge that first), design **56**. The menu fades to warm black and the world is built only once two black frames are drawn, so the long frame freezes on black, not on the menu; then five seconds of waking — veil gone by 0.8 s, hearing by 3.5, the warm grade by 4.25, **focus last** at 5 — with the camera settling onto its pose and **the colony held** (`ClockHeld`, a gate on the tick loop, never a speed, so a load wakes at its saved speed). Any key or click wakes you at once; Settings → Interface → **Wake-up** turns the dream off for a plain fade. The blur is **our own dual-filter pass** injected for the wake only, not URP's depth of field (capped at 20 px, §4a); the sound is filters on the listener (ADR 0010 amended). Nothing is left behind (`WakeUpTests`). |
| **CR** cracks | **Played twice 2026-09-26 and ready to merge — PR #243**, branch `claude/sat-wall-damage-levels-5eq8cn`, design **58**. A struck **wall** cracks in three stages at 25/50/75 % gone and a face being **mined** in six from 10 % (`CrackModel`, Hud, one drawn ladder of six), drawn by redrawing the cell's own meshes (`ChunkMesher.MeshCell`) in `Odyssey/Crack`, a multiply shader whose cracks **burst from an impact point per cell** (`OdysseyCrack.hlsl`; the first build's Voronoi tiling read as shapes); batched by mesh and level, meshed once per chunk version. **A cracked thing that comes down breaks apart** — shudder, halves, quarters, fall, sink, 1.6 s (`ChunkRenderer.Breaks.cs`, `Odyssey/Shard`, the cell's meshes clipped to quarters; §7). **And it is heard** — wood broken or taken apart, a mined face collapsing, from the owner's two recordings baked by `tools/audio/bake_demolition.sh` and played by `DemolitionWatch` (§9). The cracks **replace the mining cut slab** (kept only if the shader is missing). **A cancelled mining order now keeps its cut** (`PartMinedRock`, save section `odyssey.partmined`, no bump, hashed only while non-empty — no golden moved). Doors, sandbags and furniture join by one line in `CrackModel.Cracks`. |
...
| **HE** health | **Built 2026-09-25, merged with `main`'s medical supplies the same day, in review — PR #213**, branch `claude/relaxed-heisenberg-zxy63b`, worktree `D:\code\odyssey-health`, design `docs/design/43-health.md` (§14 as built, **§15 the merge, which wins**), plan `docs/plans/health.md`, brief `docs/reference/mockups/health-tab-brief.md`. A person's body is **six regions over the unchanged pool**; blows and falls land on regions by coverage, a limb passes its excess to the torso, injuries **merge by region and kind** (eighteen at most). Pain, consciousness, moving and manipulation are derived; **pain shock at 800 ‰** downs at 64 points; a vital region at nought downs, it does not kill (design 33 §3 still holds). Untended cuts bleed and kill through `Kill`. `CombatSystem.Hurt` is **the one owner of damage**. **The merge (§15): there is one doctor's job, design 37's `Job_Treat`, and it ends in the tend** — every injury tended at the treater's quality, every bleed stopped. `Job_Tend`, `Item_Medkit` and the branch's own Doctor and Medicine rows are gone for main's. **A bleeding colonist is a patient whatever her pool**; inside the cooldown a treatment tends and heals nothing. Two faults in design 37's driver fixed on the way: a downed patient stood up a third of the way through her own treatment and the job failed with the unit unused (a free heal each time), and a treatment reaching the cap part-way failed the same way. The ledger is **hash bit 23** (the cooldown took 22) and `HitRegion`/`FallSplit` sit on SHA-256's twentieth and twenty-first round constants, clear of the jump, cooking and ranged streams. **A bullet is a blow**: ranged combat's hits go through `Hurt` and bleed (§15f). Falls do 15 × n^1.5 (a collapse from one layer; any drop past one). The Health tab is two columns of seven on the Skills grid; condition and weapon moved to the header line. Alerts for a colonist bleeding or downed with an untended injury, and no medical supplies; debug Hurt, Heal, Kill and Give medical supplies; right-click **Tend** (a player-forced treatment). Save format unchanged; **no golden moved on the merge**. **The fight's balance moved** (43 §14b): the colony now wins every gate seed and no colonist dies; bandits bleed out. |
| **CK** cooking | **K1 built 2026-09-25, not yet played** — branch `claude/cooking`, worktree `D:\code\odyssey-cooking`, design `docs/design/48-cooking.md` (§14 is what K1 built), plan `docs/plans/cooking.md`. A **galley** (350 W) or a **campfire** (+1 wood a meal) takes **bills** on its pane — until you have N, N times, forever, five a station — and a cook fetches any raw food, 500 by nutrition (three carrots), tosses a pan and puts down a **meal**, a **vegetable meal** or, by the owner's burn curve, a **burnt meal**. The pan is the station's: a power cut or a called-away cook leaves the meal on the hob with its work banked. Eating takes the **best tier first** (meal, ration, burnt, raw), and what a colonist thinks of a meal is the food's own thought (+50, +20, −40, −50 on the 0–1000 scale; the ration keeps the old +20). The ration pack is named **Rations** now and gave `ui.res.meal` back. New save section `odyssey.kitchen`, no format bump; goldens re-baked and **measured** identical with `GoldenColonyProbe`. **POLYGON Shops is imported** (`PolygonShops/` only, 2026-09-25) and the cooker — named **Electric Cooker** by the owner — draws as its stove; a later pack now loses every prefab-name tie (`PlayScene.LaterPacks`). **Waits on the owner:** the first play. Still owed: the food in the pan going raw → cooked → burnt. Next: K2 cold store and rot, K3 hunt and butcher, K4 dining. Handle numbers will move on merge (#225, #222). |
| **BL** the bill list | **Built 2026-09-25, not yet played — PR #228** — branch `claude/bills-pane`, worktree `D:\code\odyssey-bills`, stacked on `claude/cooking` (merge that first), design `docs/design/49-bill-list.md`. The owner's mockup 20a as **one control for every station**: `BillList` draws a `BillsModel`, and what takes bills is a table (`BillsModel.Stations`), so a crafting bench is a row there. A station's pane is **680** (`.inspect--bench`); a warn strip says *No power* with the station's switch; rows carry index, product tile, name and status line, mode cell, stepper, progress and reorder/pause/remove. The pane's header and tile facts are unchanged (owner: not the tile part). |
| **SH** selection highlight | **Built 2026-09-25, not yet played** — branch `claude/selection-highlight`, `docs/design/44-selection-highlight.md`. The selected thing is lit at its own edges instead of bracketed: a white line round its silhouette, the thing itself ×1.12 brighter, a soft wash on a selected tile, and a dimmed line where it is behind something. One URP feature (`SelectionHighlightFeature`): a mask of **only the selected thing** against the camera's depth, then two blits scissored to its rectangle; **nothing selected enqueues nothing**. What is masked is the thing as drawn — a figure's renderers, the far form and items captured as `RenderActors` appends them, a building by `ChunkMesher.MeshCell` over the one cell — so it cannot drift from the picture. **The brackets are kept** as Settings → Interface → *Selection style*, and are the fallback whenever the highlight has nothing to draw or the feature is absent. **The lift multiplies; it must not blend towards white** (§6a, photographed against a control: a white blend in linear light washes orange to cream). |
| **RC** ranged combat | **Played three times and ready to merge — PR #225** (owner, after a big battle: *"great job"*), branch `claude/ranged-combat`, worktree `D:\code\odyssey-ranged`, merged with `main` 2026-09-25; design `docs/design/47-ranged-combat.md`, plan `docs/plans/ranged-combat.md`. PR #220 (the design alone) is contained in it. A pistol (Battle Royale's `SM_Wep_Pistol_Heavy_01`) on the melee line's seams: a **Shooting skill** (save format **9 → 10**, backfilled on load); a symmetric 3D integer `LineOfSight`; saved, hashed `Projectiles` landed on the tick decided at the shot (a miss carries past into the ground, a hit lands on the body, a downed pawn never takes a stray); `AttackRangedJobDriver` and fire at will; **weapon quality** for every weapon (§11); and **the reach rule** (§12): an enemy within reach is clubbed with the gun, never shot, switched by `CombatSystem.SwapByReach` alone. Handles after the merge: job **27**, item **17**, skill **8** (§13). The aim, recoil, slide, tracer, flash, dust and the owner's gunshot are presentation. **Owed**: R5 (soak with gunmen, fifty-shooter benchmark), R6 (buildings under fire), P4 (the frame with gunfire). |
| **CV** cover | **Built 2026-09-25, merged with `main`, compiled in Unity, first look taken** — PR #232, branch `claude/cool-darwin-akh02q`, worktree `D:\code\odyssey-cover`, design `docs/design/53-cover.md` (§12 as built), plan `docs/plans/cover.md`, interview `docs/research/cover-interview.md`, research `a-10-cover`. The reference's partial cover in integers — the eight cells round the target, angle bands with the diagonal ×1.75, the shooter's distance to the cover, noisy-OR, a defeated shot fired **into** the cover so it wears — plus **a descent term of our own** (low cover gone by 35°, tall by 60°). `Cover.BaseAt` is the one owner of what a cell is worth. **Sandbags** (handle 13, edifice 23, always 5 stone via `fixedStuff`) are 55 % low cover, dragged as a joined line, crossed at +150 (`NavFlags.PassThrough`, one price for both searches) and **never stood on** (`Standing.CanStandAt`; a sweep steps off anybody at rest on one, read from saved state). A stray crossing cover is caught at half its value past the dead zone; destroyed cover leaves a quarter. Bandits and undrafted defenders pick a covered spot for the first 240 ticks of an attack (`CoverPosition`); drafted colonists hold. Crouch behind cover (a derived aspect, unhashed), a *Cover* floater, a hit-point bar over **every** struck building, and a hit-chance readout at the pointer (`QueryShot` → `ShotReportView`). **First look (§13): the barricade is out** (a dim chip again, as on `main`), and **the sandbags are drawn bag by bag** — our own `SandbagMesh` laid as a revetment, stretchers in running bond under a header course, in three hessian shades (§7a-bis, research `e-12`). **No golden moved.** Embrasure, lean and repair are later units. |
| **RA** First Person | **Built 2026-09-26, compiled and tested in Unity, ready to merge — PR #242** (branch `claude/planning-session-fok6wm`, design **57**, interview `docs/research/first-person-interview.md`). A **First Person** button on a colonist's card (the owner's name, 2026-09-26; internally still *ride*) locks the view to her until Escape. The camera sits over her right shoulder and the wheel steps in to her eyes; the mouse looks round and settles back after 2.5 s. **Watch only**: no intent, nothing in a tick, a save or the hash. The in-game interface gives way to one strip at the bottom (name, activity, health, speed, *Esc Leave*, the first alert). The building is drawn as it is (walls up, roof on, no x-ray). A spring arm keeps the camera out of walls, **not doors**. Her head is shrunk by its bone when the camera is within 0.6 m of her eyes. The camera is stood **first in `LateUpdate`** from her figure's feet, and the colony view is frozen rather than moved, so a mid-ride save records it. The card's dead **Prioritise** placeholder is gone to make room. **Owed**: the frame at a level view (4K, Standard and Huge), and the rebindable key. Unity on `D:\code\odyssey-ride` after merging `main` again (the frog), 2026-09-26: compiled clean; EditMode 4,397 / 4,359 / **1 failed**, `WeaponSheathGapTests`, which is `main`'s (journal); PlayMode 161 / 144 / 0 failed. |
| **IH** inspect header | **Played 2026-09-26 and ready to merge — PR #245** (owner: *"it's great"*) — branch `claude/inspect-header-actions`, worktree `D:\code\odyssey-header`, design `docs/design/61-inspect-header.md` (Claude Design mockup 24c). A colonist's header is two large icon toggles, **Draft** (red shield, reads *Drafted* when on) and **First Person** (cyan eye), then Close over Info; the response left for the Assign tab. The icon never swaps: on, the same SVG path is filled even-odd (`PathGlyph.Solid`). Draft's face is the whole selection's (`OrderModel.DraftFaceOf`: on only when all are drafted, a red foot line when some are); First Person is live for exactly one colonist (`HudDirectors.TryFirstPersonSubject`) and has its own key at last, **Z** (`HotkeyAction.FirstPerson`), which also leaves the ride. The tile is **40, not 44**, derived from the 60 px header so the pane does not grow. |
| **AL** the Almanac | **Played 2026-09-26 and ready to merge — PR #252** (owner: *"yep it's all good"*) — branch `claude/almanac-refresh`, worktree `D:\code\odyssey-almanac`, design `docs/design/64-almanac.md`. Rewritten against the code: the mock-up's steel, pines, traits and silver are gone; 110 pages in 19 categories, keyed by registry key (name and index line are the registry's; `emit_labels.py` describes the Almanac's namespaces), links by key, the info button by the key the pane draws, and a button that finds the thing or opens its tab. **`AlmanacCatalogueTests` fails on anything the HUD can show without a page**, and `AlmanacFactsTests` holds 93 quoted numbers to the Def XML. Thirty false registry descriptions corrected. Icons: the owner's art on 16 pages, a line icon on the rest. **Every icon slot draws one table**: `IconBadge` shows the owner's art, else the key's line art in `Odyssey.Hud.IconGlyphs`, else the square — so the pane, the Almanac and every list show one picture per key, changed in one place (§2a). The issues found are rows in `docs/plans/todo.md`. |
| **BU** the Pig Butcher | **Built 2026-09-26, played once, four levels added — PR #248** — branch `claude/pig-butcher`, worktree `D:\code\odyssey-butcher`, design `docs/design/62-pig-butcher.md`, research `e-16`, `b-large-enemies-and-knockback`. POLYGON Fantasy Rivals imported (its own folder only). A hostile **kind 6** in **one cell drawn 3.64 m tall** (measured, scale 2.0), 500 hit points on `Health_Brute` (pain shock at 320), never stunned or knocked back, its cleaver its species' natural attack. **It moves as a bandit** (climbs every rock and ladder; `TraverseMode.Animal` left a stone outcrop as a safe perch, design 62 §3a) and **throws a rock** at a colonist above it or out of its reach (`SpeciesDef.hurl`, the pistol's machinery, its own clock `HurlReadyTick` in layout 7, weapon id −100 − kind, §7a). Its swing **sweeps the front arc of three** (flanks on their own dice, the facing in spare `PendingSwing` bits) and **flings** three landed blows in four two cells straight back: a **slam** into a wall or a body, a **fall** off a ledge, then 300 ticks of knockback immunity (`CombatSection` layout 6, hash bit 28; the throw's clock layout 7, bit 29). Drawn as itself (`OwnPaint`, its own box, its own portrait via `PortraitStudio.ForKind`), the cleaver in the fist (a new prop is parked at the hip; the natural one is put back every frame), a red **telegraph** on the arc during the wind-up, *Slam* floaters. **Four levels** (kinds 6–9, the pack's colourways 01_A–D, 4.4 to 5.6 m, pool 500 to 1,400, the king flings three cells) on abstract `ButcherBase` / `BruteBody`. **Debug spawn only**; no raid mix until played. As tuned even level 1 beats four bats every time (§4b). No golden moved. |
| **RD** raids | **Built 2026-09-25, reviewed 2026-09-26, ready for its first play — PR #233**, branch `claude/sharp-lamport-8q5u4h` (worktree `D:\code\odyssey-raids`), design `docs/design/55-raids.md`, plan `docs/plans/raids.md`. A band of hostiles (1–200) behaves as one. It walks on along **one** random edge over five seconds, gathers there for 2–4 game hours, probes to halfway for an hour, then assaults the **hearth** (else the colony start). A poke, or a colonist within 15 cells, starts the assault at once; at **half down** the rest leave by the nearest edge. `RaidSystem` (pawn phase, order 15) runs the clock; the schedule is drawn at the fire and saved (`odyssey.raids`, no bump). It is hashed only while a band exists, so **no golden moved**. `RaidThinkNode` sits ahead of `HostileThinkNode` and hands over to it for every fight. Mixes are Defs (`RaidMixDef`: Bandits, Gunmen, Mixed 70/30), with a new **gunman** kind (4). Debug → Events: a size slider (0 = Auto: headcount + days) and a mix dropdown. `InvokeIncident` carries size in B and mix + 1 in C. Sounds: the owner's war horn on arrival (`alert-raid-arrive`); the assault horn **was already `alert-raid`**, played by the *Raid* alert while a band assaults. **`PawnCeiling` 200 → 400**, on design 55 §11a's measurement: 200 raiders cost the Pawns phase 0.17 ms, the snapshot 0.66. **The review moved eleven things** (design 55 §15), the two a player meets first being the *Raid* alert jumping to the band's layer as a depth and raiders walking out and back between the hearth and a colonist for ever (`RaidMember.Engaged`); and it fixed one fault on `main`'s behalf, a loaded world standing in its own build's weather until the next 120-tick boundary. **The frame with 200 raiders** (640 × 480, 5070 Ti): peace 2.26 ms, gathering 4.37, assault 4.34, under the 5 ms budget. |
| **WG** world generation | **Built 2026-09-26, reviewed, played once, in review — PR #246** (EditMode 4,551 / 4,513 / 1 failed, the known `WeaponSheathGapTests` bat on `main`) — branch `claude/busy-meitner-zkrhtu` (the review, on top of `claude/sharp-euler-a6xtci`), design `docs/design/59-world-generation.md` (57 on the build branch; `main` took 57 and 58 first), Claude Design's spec `docs/reference/mockups/world-screen-spec.md`, interview `docs/research/world-generation-interview.md`, research `a-13`, plan `docs/plans/world-generation.md`. New game opens on a **World** screen: a seeded **128 × 64** hex planet wrapping east–west (**12 ms** to generate; 64 × 32 until the owner's first look asked for "at least 4x", design 59 §4e — warped coasts, a sea share of 35–55 % by seed), six biomes (owner's ruling on the spec: Ocean, Meadow, Cold steppe, Dry scrub, Marsh, Ice), hill bands **Flat, Rolling, Hilly, Mountainous, Sheer** (ours, not the reference's), region names, zoom 1–8× and pan; one mipmapped texture per seed at 1.5× (**203 ms** Debug, measured against 332 at 2×), **released with the planet whenever a colony goes live** (`ReleaseWorldMap`). The site panel's temperatures are red / amber / green (`HudTheme.SiteTemperature`, §4f). **Only Meadow can be settled.** The site sets the board's **hills** (`SiteBoard`; Rolling is today's board; Mountainous is 24 layers, +48 % memory, 110 MiB on Huge) and its **climate, seasons and rain** (`SiteClimate`, `SiteRules`: the 53° / 9 °C / 1,000 mm reference site is `Climate_Temperate` exactly). **A request with no site builds exactly today's board**, asserted field for field, and **no golden moved**. Save format **10 → 11**: the header carries the tile whole. `SaveFixtures.AsFormat` is how a test makes an older save now. Factions, rivers, roads and coast are recorded seams. **The review** (journal) moved the map's finish to the row alone (the date line drew as a dark valley), made the pole notches pickable, fixed a PlayMode test that typed into the hidden seed box, freed the map texture, and pinned §5's table (a Hilly board had Rolling's caverns). **Open for the owner:** `annualMeanC` is the season curve's anchor, up to 4.3 °C under the year's mean (design 59 §7). |
| **TR** trading | **Built 2026-09-26, reviewed and merged with `main` the same day; not yet played or run in Unity** — branch `claude/wonderful-clarke-kkd9pq`, design `docs/design/65-trading.md` (§10 as built, **§12 the review**; numbered 57 until the merge, since `main` took 57–64 meanwhile), plan `docs/plans/trading.md`, interview `docs/research/trading-interview.md`, the owner's window spec `docs/reference/mockups/trade-window-spec.md` (28a Sell / 28b Buy). **Gold** is item 18 (stack 500, category Items; `ui.res.credit` renamed `ui.res.gold`), and every item has a `marketValue`. A **Visitor** faction (`PawnFlags` widened to `ushort`), the **trader** kind **10** (after the butcher's four) with its own mind (to the hearth, idle at the fire, out by an edge), and **`Incident_Trader`** (debug Events), with the raid's edge choice lifted into `EdgeArrival`. `TradeSystem` keeps the visit (a day, paused while negotiating; a raid sends it home) and applies a deal **whole or not at all** (`TradeLine`s then `TradeCommit`); prices have one owner (`TradePricing`, ×0.6 / ×1.4); the colony trades from its stores plus loose stacks within four cells of the trader, and bought goods land **inside that same four cells**. Right-click **Trade** sends a colonist (`Job_Trade`, 28); the **pausing** window (`TradeModel`, `HudShell.Trade.cs`, `ModalHeld`) is the owner's Sell/Buy spec. **The review's one behavioural fix**: an applied deal ends the session in the simulation and the window *waits* for the first publish after Confirm (`WorldSnapshot.Generation`, new) — gone means applied and it closes, still ready means refused and it stays up saying so; before, Confirm sent a `TradeCancel` and closed at once, so a refused deal vanished silently. Harm: accidental → home, ordered attack → `TurnedHostile` (hash bit **30**; `main` took 28–29). Save: `odyssey.trade` layout 2, no format bump. **No golden moved.** Fast tier green on the merge; the view is uncompiled until Unity runs. |
...
| Butterflies, the night glow, the Butterflies ladder | `docs/design/52-ambient-butterflies.md` |
| **The Almanac: its pages, the tests that keep them true, icons** | `docs/design/64-almanac.md` |
| **Tree species, bushes, berries, mushrooms, loose stones, the topple** | `docs/design/45-nature.md` |
...
| **The clouds: the rings, cover as height, their colours, when nothing is drawn** | `docs/design/63-clouds.md` |
...
| The shoreline, the water's colour, marsh painting, the ground field | `docs/design/38-meadow-overhaul.md` §24 |
| **Jumping a one-cell stream, the jump's price, a short jump, `JumpArc`** | `docs/design/46-jumping.md` |
| Animals, the species and kind, the animal mind, the computed walk | `docs/design/29-animals.md` |
| Medical supplies, the doctor, patients, self-treatment | `docs/design/37-medical-supplies.md` |
| **Cover, sandbags and how they are drawn, pass-through cells, crouching behind cover** | `docs/design/53-cover.md` |
| Drafting, orders, combat, health, weapons, the Sword Combat clips | `docs/design/33-combat.md` |
| **The inspect pane's header: the Draft and First Person toggles, Close and Info** | `docs/design/61-inspect-header.md` |
| **Health: the six regions, afflictions, bleeding, tending, fall damage, the Health tab's brief** | `docs/design/43-health.md` |
| **Bandits: the outfit layer, the gang's rows, the vest's two rectangle sets, the helmet** | `docs/design/42-bandits.md` |
| **Walls down, stumps, hiding the storeys above, what counts as build mode** | `docs/design/42-walls-down.md` |
| **The home area, keeping a colonist home, the Assign tab, `Reachable` against `CanTravel`** | `docs/design/43-home-area.md` |
| Which animals a world generates, the census, arrivals and departures, the night, the frog's bank and hop | `docs/design/30-wildlife.md` (§8 for the frog) |
...
| **The selection highlight, the mask feature, what a selection is drawn with** | `docs/design/44-selection-highlight.md` |
...
| The title screen's bed, and the hand-over into a world | `docs/design/17-start-flow.md` §12, and design 56 |
| **The wake into a world: the fade, the build behind the veil, the dream's blur, grade and muffle, the camera settle, the clock hold** | `docs/design/56-wake-up.md` |
...
| **Guns, bullets, line of sight in three dimensions, the aim stance, the tracer** | `docs/design/47-ranged-combat.md` |
| **Raids: the band, its phases, the gunman, the mixes, the debug raid controls, the horns** | `docs/design/55-raids.md` |
| **The planet, world sites, biomes, what a site does to the board and its climate, the World screen** | `docs/design/59-world-generation.md` |
| **The Pig Butcher, sweeps, flings and slams, knockback immunity, a hostile kind drawn as itself** | `docs/design/62-pig-butcher.md` |
| **First Person (ride along): the shoulder camera, the eye stop, the strip, what a ride puts aside** | `docs/design/57-ride-along.md` |
| **Cracks on walls and mined rock, the crack shader, the break into quarters, part-mined rock keeping its cut** | `docs/design/58-cracks.md` |
| **Trading: the visit, the neutral side, value and price, the ledger, gold** | `docs/design/65-trading.md` |
...
- **Fast tier** (`scripts/test-fast.sh`, ~55 s, no Unity): **1,792 Sim + 1,180 Hud**
...
**The fix list is `docs/plans/todo.md`** (bugs, half-built things, art, stale comments, each with its evidence); this section keeps only the gaps a session must know before touching the code.
...
- **Health is a body, not yet a medicine chest** (2026-09-25, design 43 §14d): no scenario carries
  a medkit and nothing makes one (the debug menu gives them), a standing patient walks on while she
  is tended, and cold, heat and hunger still only slow a colonist — their lethality is the next
  unit. Infection, scars and surgery are recorded, not built.
