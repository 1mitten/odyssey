...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-21 on
  `claude/huge-map` **after merging main and after the decoration measurement**: EditMode
  **2,319 total, 2,296 passed, 0 failed**; PlayMode **93 total, 88 passed, 0 failed**. The two new
  EditMode ones are `SurroundCostTests`, the guard that the surround is charged to itself and not
  to the board; the new PlayMode one is `FrameTimeTests.TheDecorationAgainstTheFrame`. On the merge
