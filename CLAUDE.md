...
| **FR** the frog | **Built 2026-09-25, not yet played** — branch `claude/frog`, design 30 §8. The culvert frog, kind 4: the owner's `Frog.fbx` (Quaternius, no walk clip) **hops** on its looping Jump clip, with the drawn position paced to the clip's measured flight (`hopGait`, `HopSurge`) so it sits, leaps and lands rather than gliding. **A bank habitat** (`WaterBank.Near`, one owner): the meadow seeds frogs in twos to fours on the banks of its water, every leg ends within three cells of water (`SpeciesDef.bankRadius`), a stray walks back, arrivals come in only where water meets the edge; meadow density 15 → 21 so hogs and rats keep their share. Does not swim (a sixth traverse mode is a flood per rebuild); stays out in the rain (`ignoresRain`). Played board golden re-baked, probe clean. |
