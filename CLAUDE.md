...
- **The colony builds (U26, first slice, 2026-09-17). Design, test procedure and what is
  still open: `docs/design/15-building.md` — read that before touching this line.** Open the
  Build palette with **B**,
  ...
  **Blueprint and frame are not two records:** a site is a cell with four numbers on it, so
  ...
  designation kind nothing acts on, and — reported by the owner and **unreproduced** — walls
  are still not going up in the running game after the composition fix. The rejection log added
  the same day exists to settle that on the next run rather than by a fourth round of inference:
  press Play, order a wall, read the Console, and match the line against the table in section 6
  of the design doc.
