...
| **PS** prisoners | **Built 2026-09-26 (P1–P12), fast tier green, not yet run in Unity or played** — branch `claude/prisoner-bed-assignment-98afc0`, design `docs/design/59-prisoners.md` (**§15 is as built, and lists where it departs**), interview `docs/research/prisoner-interview.md`, research `a-20`, plan `docs/plans/prisoners.md`. Marking a bed *for prisoners* makes its enclosed room a **cell** (a prison bed with no room **shackles**); prisoners walk in `TraverseMode.Bandit`, so a door holds them unless somebody holds it open. **In** by capture (right-click a downed enemy: a menu, *Capture* / *Finish off*, replacing the instant execution), a warden's capture mark, **surrender** (a raider under 30 % may yield if a cell is free) and **arrest** (a command on a colonist's own pane — not a right-click, which stays a move). **Warden** work: capture, feeding, recruiting, letting go. A **visible willingness bar** with its hours and blockers (`Recruitment.Factors`), a **visible escape risk** with its reasons, × 1/√n (`EscapeRisk.Odds`), an escapee **bashes the door**, runs for the edge, and is a prisoner again if downed. Fates: Hold, Recruit, Release, Exile (the pane's mode row cycles them); an arrested colonist released goes back to work. **`Kind` is never rewritten** — custody (`Pawn.Custody`, hash bits 28–29) and one owner of sides (`Allegiance`); **one bed rule** (`BedRules`). The only golden bake was P3 (Social, format 10 → 11, Warden). Debug → Spawn: Imprison, Free, **Break out**. |
