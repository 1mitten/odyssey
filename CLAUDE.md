...
| **CF** the campfire | **In review — PR #170**, branch `claude/campfire-art`. The campfire draws as Western Frontier's prop, burns, smokes, lights the ground at night and crackles (`FireDirector`, `LoopEmitters`); radiant heat is a hot tile and a warm ring with no per-cell field. **Idle colonists gather at it** (two in three), **stay** once there, and a bedless sleeper lies beside one. **They sit** (2026-09-24, §18d): half of all settles at the hearth are a seat — a `Wait` whose `DestCell` names the fire, so saved and hashed with **no format bump and no golden moved** — turned to face it. **The crouch that drew it read as sneaking and is off** (§18e): seated colonists stand, and a floor-sit clip of our own is owed — the blend, the catalogue field and its test are in, dormant. **Merged with combat 2026-09-24** (§19): marauders are kept off the hearth, `Seated` stays a bool beside `Asleep` because `PawnFlags` is full, and the right column went **296 → 271** because combat's health bar spent the same coverage — the owner's choice over raising the ceiling. **A bare `PlayScene.RebuildCatalogue` wipes every colonist's swatches**; `CharacterSwatches` is the second pass. `docs/design/31-campfire-art-and-fire.md`. |
...
| **CM** combat | **C1 merged (PR #176). C2 health and melee, C3 weapons and three playtest rounds: in review, PR #180, branch `claude/combat-c2-final` (`D:\code\odyssey-combat-drawn`), merged with `main` 2026-09-24.** Owner, after a battle with a crowd: *"it seems great ... happy to merge in"*, 3.5 ms a frame. Hit points (downed at 0, dead at −50 %), corpses, a debug-spawned marauder, right-click attack, a context menu to equip, four weapons sheathed at the hip and drawn for a fight, the lock-on ring, no shared tiles, hit reactions, criticals and knockback, health on the cards, swing/slice/thud sounds, spreading spawns, *Arm every colonist*. Combat's jobs are handles **17–21** after power's. **An unordered fight ends in downs, never deaths**, and a downed colonist cannot heal until rescue (C4). Next: blood, then C4–C7. `docs/design/33-combat.md`, plan `docs/plans/combat.md`. |
