...
| **WT** the Work tab | **Built on `claude/happy-tesla-2onz0q`, PR #145, both tiers green** — `docs/design/27-work-tab.md`, from the owner's supplied mockup re-pointed at our tokens (`docs/reference/mockups/work-v1.html`). **The tab opens and works**: F1 or the bar item, a click cycles a priority, right-click cycles back, shift sets the column, every change a `SetWorkPriority` intent that lands while paused. Twenty-two `ui.work.*` columns with the simulation's four live and eighteen drawn as **not built yet** — a third state, deliberately not the incapable grey; rotated headers at **−66°**, the angle our 34px pitch demands rather than the mockup's −62°, asserted in the fast tier; the five-step skill border, the passion flames, and a Simple / Detailed switch that moves nothing but the glyph. **`HudKey` gained its first function key** — F1 was a legend on the bar since the bar was built. Drag-paint is the one gesture not built (OQ-W5). |
...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-20 on the events
  ...
  77 passed, 0 failed**, with `HudSmokeTests` now naming thirteen framed regions (the Events panel joined). **PR #145 is green on both tiers with a fourteenth**, the Work tab; its counts are not recorded here because that run was CI's rather than a local one.
