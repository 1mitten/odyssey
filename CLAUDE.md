...
| **AB** ambient butterflies | **Built 2026-09-26, not yet played, draft PR #234** (PlayMode and the contact sheet owed) — branch `claude/ambient-butterflies`, worktree `D:\code\odyssey-butterflies`, design `docs/design/52-ambient-butterflies.md`. Procedural butterflies over open grass near the camera: an engine-free state machine in `Odyssey.Hud` (`ButterflyMeadow`, the birds' shape) behind a habitat seam the render mirror answers, one structured buffer and **one `RenderPrimitives` call by day, two at night**. Wings are shader tables, flapped ~10 Hz with a 42% downstroke; they land on flowers, bask, scatter from colonists within 2.5 m, thin with cloud, leave in rain, none in Rime. **At night each glows in its own hue** (cyan, magenta, amber, violet, colour-blind checked) with a drawn halo and a light read off the depth texture — never a URP light, and the wing stays under bloom's threshold. **Real seconds, not game time**, or speed 3 strobes. Graphics → Detail → **Butterflies** Off / Few / Many / Swarm (0/80/200/500), owned by the presets. Nothing in a cell, a save or the hash. |
