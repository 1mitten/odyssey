...
| **PS** prisoners | **Designed 2026-09-26, nothing built** — branch `claude/prisoner-bed-assignment-98afc0`, design `docs/design/58-prisoners.md`, interview `docs/research/prisoner-interview.md`, research `a-20`, plan `docs/plans/prisoners.md` (P1–P12). Marking a bed *Prisoner* makes its enclosed room a **cell** (a prison bed with no room holds its prisoner **shackled**); prisoners walk in `TraverseMode.Bandit`, so a door holds them unless a colonist holds it open. In by capture (a right-click **menu**, Capture / Finish off, replacing today's instant execution), a warden's capture mark, surrender and arrest. A **visible willingness bar** (warden Social × mood × treatment, ETA and blockers) instead of a hidden roll; a **visible escape risk** with its reasons, × 1/√n so a big prison is not punished; an escapee bashes the door. Recruit, Release, Exile first; Ransom a seam; Execute and labour later. **`Kind` is never rewritten** — a sparse custody override (`Pawn.Custody`, hash bits 28–29) and one owner of sides (`Allegiance`); **one bed rule** (`BedRules`) before anything else. The only golden bake is P3 (the Social skill, format 10 → 11, and the Warden work type). **Phase gate: waits for the owner's approval of the plan.** |
