...
| **FT** faces and talking | **Built 2026-09-26, not yet played — PR #247**, branch `claude/face-expressions` (worktree `D:\code\odyssey-faces`), design **59**, research `e-15`. Every live colonist **blinks**; the face **follows context** (`FaceContext`: stern drafted or fighting, alarmed fleeing, pained hurt, tired below rest 280, glum below mood 350, eyes shut asleep) on the rig's one `Eyes` and one `Eyebrows` bone, written **absolutely from rest** by `FaceRig`; two colonists **talk** in turns with five manners (emphatic, question, musing, tilt, shake), listener replies, an eyebrow flash on a greeting, and **talking hands** — the one part that reads at play zoom. Colonists at leisure **standing still** within 4 m fall to talking by themselves (measured: never on the move). **No mouth** (the owned lips cannot open without tearing the head). The debug menu's **Faces** tab forces a face or hands it back to context, and starts a talk. Logic engine-free in `Odyssey.Hud`; 19 us a frame for 64 figures. `FaceLiveTests` proves an expression, a drafted stern face, a talk and a blink on a live colonist. Nothing in a cell, a save or the hash. |
...
| **Faces: expressions, the blink, talking, `FaceRig`, the Faces debug tab** | `docs/design/59-faces-and-talking.md` |
