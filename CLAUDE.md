...
| **M3** build and dig | **Under way.** Designations, felling, stockpiles, mining, walls, deconstruction, floors and collapse, paving, ladders and beds are all in. Stairs (`U44`, design 63) built 2026-09-26 on the deep-mining branch, not yet played. The gate is a ten-day headless run. |
...
| **DM** deep mining | **Designed 2026-09-26, nothing built; the plan waits for approval** — branch `claude/intelligent-davinci-rbnjm7`, design `docs/design/62-deep-mining.md`, plan `docs/plans/deep-mining.md`, interview `docs/research/deep-mining-interview.md`, research `a-12-ore-by-depth`, `a-04-cave-ins-and-prospecting`. **One deeper board, 32 layers**, made cheap by never touching rock nobody has opened: solid terrain loses its unused navigation region (the only depth-scaled per-edit cost), the underground draw walk stops a fixed depth below the slice, and the door list rescans by chunk. Then copper, gold, gems and **Emberquartz** in depth bands beside iron and coal, a slower **deep stone**, larger deep caverns, **fog drawn as plain rock** (which also fixes the inspect pane naming undiscovered ore), a prospecting job, **cave-ins that warn and down but never kill** with a mine prop, a smelter burning coal or wood for iron and copper bars, and soft ground reading *Dig*. **Stairs (U44) are the prerequisite**: a hauler cannot climb a ladder. |
...
| **Deep mining: the 32-layer board, why rock is free, ore bands, fog, prospecting, cave-ins, the smelter** | `docs/design/62-deep-mining.md` |
...
**The board is a wooded meadow**, 120 × 120 × 32 since 2026-09-26 (`GridSize.OfferedLayers`, DM2; 16 before) — grass, woodland with a clearing at the start,
...
- ~~A hauler cannot climb a ladder~~ — **stairs (`U44`, design 63) are built as of 2026-09-26**, not yet
  played: a hauler carries a load up a two-cell flight. A ladder still refuses haulers, by design.
