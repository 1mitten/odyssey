...
| **FC** frustum culling | **Built 2026-09-21, branch `claude/frustum-culling`, OFF by default and not merged.** `ChunkRenderer.Render` walked every chunk of every drawn layer with no frustum or distance test, which is where a big board's frame went (`docs/design/28-map-size.md` §9). At the real 40 m shadow margin Huge culls **317 of 443 chunks (71.6%)**, **8.43 -> 4.57 ms** and **5,392 -> 2,053 draw calls** — under the 5 ms budget; Standard saves little (31.7%), which is the tell that it measures the board rather than the machine. **The margin is load-bearing**: without it 93.7% is culled and the frame reads 3.35 ms, but off-screen walls stop casting into view and shadows pop as the camera pans, so **3.35 is not a shippable figure and must not be quoted**. The bound is the shadow distance, not the sun angle. **Blocked on its own proof**: `CullingDoesNotChangeThePicture` failed on its control (a frustum admitting nothing moved 3.22% of pixels, so the capture is not seeing the board) and the flag stays off until it passes — see `docs/bug-patterns.md` P14, which is the reusable half. |
...
- **Fast tier** (`scripts/test-fast.sh`, ~20 s, no Unity): **915 Sim + 583 Hud** (2026-09-21,
  `claude/frustum-culling`); Long tier **34**, up from 23 because the per-board measurement arms
  all carry `Category("Long")`.
...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-21 on
  `claude/huge-map`: EditMode **2,285 total, 2,262 passed, 0 failed**; PlayMode **92 total,
  87 passed, 0 failed**. The run before it, 2026-09-20 on
