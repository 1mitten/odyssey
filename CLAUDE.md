...
| **HT** hardening | **Plan approved 2026-09-25; HT1 built, in review — `claude/ht1-local-districts`.** The baseline audit is `docs/audit/2026-09-19-baseline.md`; the ordered units are `docs/plans/vertical-slice.md` §HT. **HT1, the navigation rebuild is local**: portal edges and adjacency kept as links come and go, districts repaired where an edit touched them; one mined cell at the scale target **1.795 → 0.184 ms**, goldens and every path unchanged, an in-graph oracle over randomised edits (design 05 §7). **It found HT10**: with the navigation local, the busy tick's board-scaled cost is the temperature line's enclosure solve (8.0 ms on the scale-target lattice) and Needs (4.2 ms); `main`'s edit tick there was 63.8 ms, not the audit's 1.19. |
