...
| **CD** the draw | **Built 2026-09-24, awaiting its first play** — branch `claude/character-draw`, PR #193, worktree `D:\code\odyssey-draw`, design `docs/design/41-the-draw.md`. **Traits** are real: eighteen, each moving a number the game already has through one aggregator (`Pawn.TraitFactorPerMille` / `TraitOffset`, the only readers of `Pawn.Traits`), saved and hashed, shown on the setup page and on the inspect pane's Needs tab. **Every colonist the game makes rolls Standard** — exactly 12 levels over the five live skills, one major and one minor passion, one good and one bad trait — so **every golden moved with behaviour** (probe in `Golden.cs`); `Legacy` keeps old saves rolling as they did, and mechanic fixtures that pin exact numbers ask for it through `ScenarioDef.colonistProfile`. **Gamble** is one pull per colonist from long-tailed tables — measured over 100,000: +32%, 9.0% duds, 6.7% with a 12+ skill — revealed on a fruit machine built into the setup page from the owner's Claude Design spec (departures tabled in §6.7): `ReelMachine` lands every reel exactly by writing the value where constant braking brings it to rest, out of view. Pulls stick through Back; Start waits for three landed. Sound is wired and silent until clips exist (`odyssey.sound.draw.*`). |
...
| **Traits, Standard and Gamble rolls, the reel machine** | `docs/design/41-the-draw.md` |
