...
| **TM** traits and mental health | **Built 2026-09-25, reviewed and merged with `main` again the same day, awaiting its first play — PR #215**, worktree `D:\code\odyssey-traits` — branch `claude/peaceful-lamport-58fozw`, design **51** (renumbered from 44, which the selection highlight holds; §8a is the review), plan `docs/plans/traits-and-mental-health.md`, interview `docs/research/traits-interview.md`, research `a-18`, `a-19`, `b-mental-health-models`, `g-04`. **The mood band is the simulation's** (a rested colonist reads Content; the interface copies no threshold). **The Thoughts tab is live**: every thought named, Now then Memories, worst first. **Traits**: two or three from the colonist's own seed, dealt on the first tick (no `Generated` golden moved), saved in `odyssey.pawn.mind`, reading mood, the break line, learning, work speed and work she will not do; shown under the needs, on the select card and greyed on the Work tab. **The thirteen traits are a placeholder set, every number INVENTED, until the owner fills the interview's table.** **Three break lines** (4 / 0.8 / 0.5 days) and **sulk, binge, tantrum, berserk** beside the wander, each an Events row. `ScenarioDef.traits` turns traits off for a fixture about something else. Found and not fixed: `PawnPurpose.AnimalMind` and `DeconstructRefund` share a salt. The merge fixed two faults of `main`'s that it exposed — the weather's temperature offset lost on a load, and a stunned pawn's path never re-asked after one (`docs/bug-patterns.md`). **The Thoughts tab is rebuilt to mockup 23b** (§10): mood column with meter and breakdown, thoughts table with the pager, traits strip; **every tab's body is 244 px now**. **And no colonist in the game had ever been dealt starting skills or traits** — the first-tick pass asked for tick nought and the scene wakes at noon; fixed with `SimWorld.StartTick` (§10a). A starving sleeper wakes to eat (owner). |
...
| **Traits, mood bands, the Thoughts tab, mental breaks** | `docs/design/51-traits-and-mental-health.md` |
