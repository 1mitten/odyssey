...
| **CR** cracks | **Built 2026-09-26, not yet compiled in Unity or played** — branch `claude/sat-wall-damage-levels-5eq8cn`, design **57**. A struck **wall** and a face being **mined** crack in three stages at 25/50/75 % gone (`CrackModel`, Hud), drawn by redrawing the cell's own meshes (`ChunkMesher.MeshCell`) in `Odyssey/Crack`, a multiply shader with generated, world-space cracks; batched by mesh and stage, meshed once per chunk version. The cracks **replace the mining cut slab** (kept only if the shader is missing). **A cancelled mining order now keeps its cut** (`PartMinedRock`, save section `odyssey.partmined`, no bump, hashed only while non-empty — no golden moved). Doors, sandbags and furniture join by one line in `CrackModel.Cracks`. |
...
| **Cracks on walls and mined rock, the crack shader, part-mined rock keeping its cut** | `docs/design/57-cracks.md` |
