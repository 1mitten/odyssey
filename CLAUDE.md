...
| **IL** the illustrated look | **Grass built 2026-09-22, not yet seen by anybody** — branch `claude/illustrated-look`, `docs/design/29-illustrated-look.md`. The owner supplied four references and three of them turned out to be **this project's own concept renders**, never committed since 2026-09-15 — and they are ink-outlined flat comic art, which is what `06-rendering-and-camera.md` §1 spent a year saying they were not. So there are two targets and they disagree: the concept renders are a *graphic* look and the shipped golden hour is a *photographic* one. **Owner call: build both behind `Graphics ▸ Look ▸ [ Lit \| Illustrated ]`** and judge by flicking between them; ranked grass, sky and clouds, ink, with banded colour deferred as more work. **Grass is the first unit and is in.** The meadow was three Synty cut-outs that `EnsureScatterModules` dropped when the art was missing, so a clone without the packs — including the runner — drew bare ground and no test could see the largest visible thing in the game. `GrassMesh` builds the clumps in code, `Odyssey/Grass` colours them root to tip and bends them in the vertex shader, and the one non-obvious part is the **lean towards the camera**: a blade is a thin upright card and a camera at 48° sees its top edge. Two near-misses are recorded in §2.4 — the material swap must key on the module's *shape*, because crops carry the foliage tint too, and `StuffPalette.FoliageTints` had to be retuned because the old numbers multiplied blue past 2.0 to get green out of straw. **Played once 2026-09-22** and three tunings came back — lighter, thicker, and density 60 → **140** (§2.7). Priced with a control in one run: the shipped meadow is **+0.26 ms** and doubling it again to 280 is *inside the noise*, nine draw calls for twenty-one thousand instances. That second run also showed the first one's "unexplained non-linearity" to have been noise throughout — a pairing cancels what the machine does *between* its halves, not *during* them. The density had **five owners** writing the same literal and now has one. |
...
| Grass, the wind, the Look setting, banded colour | `docs/design/29-illustrated-look.md` |
