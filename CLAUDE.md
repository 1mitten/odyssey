...
- **Fast tier** (`scripts/test-fast.sh`, ~20 s, no Unity): **1,008 Sim + 696 Hud**
  (2026-09-22, `claude/temperature-core` merged with main); Long tier **34**,
...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-22 on
  `claude/temperature-core` **after merging main**: EditMode **2,524 total, 2,498 passed,
  0 failed**; PlayMode **100 total, 95 passed, 0 failed**.
  **Run PlayMode, and say so.** That branch reported three green tiers and no PlayMode figure,
  and PlayMode had a real failure sitting in it: `EveryPaletteKeyHasItsOwnShape` on the campfire,
  live on the Build palette with no glyph, drawing the placeholder square the specification
  forbids. Nothing else could see it — `PaletteGlyphs` is in Presentation, which the fast tier
  does not compile, and EditMode does not carry the test. **A tier with no number beside it was
  not run**, which is the Long tier's lesson from PR #145 in different clothes
  (`docs/lessons.md`). On the run before it, 2026-09-21 on
