...
| **NW** nobody in a wall | **Built 2026-09-21, not yet played** — `docs/design/28-nobody-in-a-wall.md`. A build site is walkable up to the instant the building exists and `Raise` asked nothing about who was standing there, so a colonist crossing the cell was sealed in **for ever** (no path can start in an unwalkable cell or end in one). Three parts, the owner's choice: a **detour** (`NavFlags.BuildSite`, `MoveCost.SiteDetour` 120) keeps passers-by out; the **guard** in `Raise` waits for somebody walking through and moves somebody standing still, asked by the driver through `CanRaiseNow` **before** the success roll so waiting never costs a botch; and `TrappedPawnSystem` **sweeps** every tick, which is the only thing that frees the colonists already walled up in existing saves. `PawnEviction` is the one owner of where a displaced colonist goes. Goldens did not move, measured on all three seeds. |
| **BA** build appearance | **The frame after a build, 2026-09-21.** The owner reported a one-to-three second delay before a built wall or door appears. Measured in a real player loop (`BuildAppearanceTests`) rather than reasoned about: **the publish seam is innocent** — the wall is in the render mirror on the next frame and drawn on the one after. What the probe found instead was **P14**: `WorldRenderModel.Version` was one number for the whole board, so one wall re-meshed all 45 drawn chunks — **12.53 ms in that frame against 0.7 either side**. Per-chunk versions (`ChunkVersion`) make it 3 chunks and 1.73 ms. **The seconds are still open**: the leading candidate is the editor's asynchronous shader compilation, which a batch run cannot reproduce, and the experiment is one toggle in a Play session (`06-rendering-and-camera.md` §6c.3). **A cell edit must now dirty every chunk whose mesh depends on it** — the global version was forgiving under-marking. |
...
| Who may stand where a building goes, the eviction rule | `docs/design/28-nobody-in-a-wall.md` |
...
- **Unity tier** (`scripts/unity.sh test editmode`, authoritative), last run 2026-09-21 on
  `claude/build-appearance-and-entombment`: EditMode **2,275 total, 2,255 passed, 0 failed**. The
  run before it, on `claude/mark-pass-batching`, is below.
- **Unity tier** (earlier), last run 2026-09-20 on
