...
| **HR** highlights reel | **Designed 2026-09-26, nothing built** — branch `claude/charming-edison-8xso6q`, design **63**, plan `docs/plans/highlights-reel.md`, interview `docs/research/highlights-interview.md`, research `b-highlights-reel.md`. The owner chose **true replays**: a saved, unhashed **chronicle** (deaths, downs, close calls, raids, milestones; the sentence written when shown), an **order log** hooked at `SimWorld`'s intent handler (paused orders included), the daily autosave copied off the main thread as the keyframe, and a directed camera over a letterbox. A highlight from another build is a **"then and now" card**, since a replay only plays on the build that recorded it. **HR0, the spike, built 2026-09-26** (design 63 §10): `SimWorld.ReplaySink` hooked at the intent handler, `ComputeLightHash` (the full hash without the grid, 0.54 ms against 23.2), `ReplayLog`/`Replayer`, and the editor recording every session into `Logs/replay/` by default (Debug → Cheats → *Replay recording*). `ReplayTests` replays a mid-run session with paused orders exactly, full hash included, and catches a dropped and a moved order. **Waits on the owner: play a day in the editor, then `dotnet run --project tools/dotnet/Odyssey.ReplayProbe`.** Presentation half not yet compiled in Unity. |
...
| **The highlights reel: the chronicle, the order log, keyframes, replaying a moment, the "then and now" card** | `docs/design/63-highlights-reel.md` |
