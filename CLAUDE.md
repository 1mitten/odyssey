...
- **Phase 4 (execution): M0 complete, M1 under way on branch `claude/m1-world`.** U01-U07 done, and the whole simulation stack runs: grid, support solver, two worldgen paths, pathfinding, pawns, save/load, Def loader, subsystem schedule, and the snapshot-read / intent-write seam. M1 presentation has instanced chunk rendering (no GameObject per cell), a slice camera rig, click-to-inspect and a generated play scene (`scripts/unity.sh exec Odyssey.EditorTools.PlayScene.Build`). **278 EditMode tests (276 green, 2 skipped) plus 2 PlayMode** (1.7 s fast tier, 37 s Unity gate, `unity.sh test playmode` for frame time). **Frame time is measured only by `FrameTimeTests` under the real player loop** — wooded meadow 0.41 ms, city 1.48 ms on the RTX 5070 Ti at 640 × 480 (2026-09-16, grass to the rim and trees) — never by an editor `camera.Render()` loop, which measures its own history (`docs/lessons.md`, "Benchmarking the renderer"). M1 report: `docs/milestones/M1-report.md`. Remaining in M0: **U08 CI**, half done: `.github/workflows/ci.yml` runs the fast tier, the Long tier, the content-registry check and the icon tooling tests on GitHub-hosted Linux for every push and pull request, so those are gates now; the Unity tier in the same file waits on the owner registering the self-hosted Windows runner (labels `self-hosted`, `windows`, `unity`) and setting the repository variable `UNITY_RUNNER=1`, then branch protection on `main` requiring both checks. Plan: `docs/plans/vertical-slice.md`.
...
- **Mining, rock and caverns, 2026-09-16** (`docs/research/mining-interview.md` — the interview, the plan and what departed from it). **The board now has a mine under it.** `MakeWooded()` is a cover mode rather than the barren board with trees put back, so terracing, rock outcrops, ore and caverns all reach the board that is played: surface terraced across y8-12, 23 outcrops, 4 sealed caverns, 100 ore deposits in ~83,000 cells of rock. **Depth is what is left after headroom** — the ground sits as high as it can while leaving `headroomLayers` (3) of sky above the tallest terrace, which on the 16-layer board puts it at 10. It used to sit two fifths of the way up, leaving two layers of rock, so coal (band 7-22 cells down) *could never generate at all*; `NaturalDepthTests` measures that now. **Caverns** (pass 6, before ore) are sealed: no mouth, carved strictly inside each column's rock band, found by mining into one, with every third ore deposit hung on a chamber wall. **Ore is invisible until a face is cut in it** — `CellFlags.Discovered`, a one-way latch set on the six solid neighbours of a mined cell, with the render mirror substituting plain rock for an undiscovered seam so the module, the colour and the cyan trim all agree; nothing is discovered at generation, not even cavern-wall ore. **Mining is the second job that edits the world:** `MineWorkGiver` (work type *mining*, scanned after cutting and before hauling) and `MineJobDriver`, priced per material from `TerrainAt(terrain).workToClear` (rock 700, iron 900, coal 760) rather than one number on the job def. A seam always yields `OrePerCell` (15, **ASSUMED**); plain rock yields `StonePerRock` (10, **ASSUMED**) one cell in `StoneChanceOneIn` (4, **ASSUMED**), rolled from (world seed, **cell index**) so the answer belongs to the cell and survives a save, a reload and any re-ordering of the colony's work. **Bedrock refuses the order** rather than quoting its 2,400 ticks, and so does the ground under a standing tree. **A dug shaft is climbable:** the dig registers a Ladder connector to whichever vertical neighbour is open, and `NavGrid.RefreshFrom` now treats a declared connector footprint as its own floor — without that a shaft's middle cells rest on nothing, are not walkable, and the portals at both ends of every ladder have nothing to join. A colonist that digs the cell under its feet steps down into it. Names: `ui.res.stone`, `ui.res.ironore`, `ui.res.coal`, all three art gaps. **Not yet:** the free ladder is a placeholder for a built one, mined cells and ladders are not in the save (OQ-08 again), mining collapses nothing (U29 keeps that), and a sealed cavern is still visible if the player scrolls the layer down — there is no fog of war, so what the ore-sight rule protects is the treasure, not the chamber.
